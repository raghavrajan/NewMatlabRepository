function [Bout] = BoutRhythmSpectrogram2(DirectoryName, FileList, FileType, PlotOption, ThreshMultiplier, MotifTemplate, ChooseBouts, StretchValues, GetRidNoise)
...
while (ischar(SongFile(1)))
   ...
   try
    ...
    if (strfind(ChooseBouts, 'no'))
        ...
        if (onsets(1) > 0.5)
            BoutOnsets = [onsets(1); onsets(find(Temp > 0.5) + 1)];
            if (offsets(end) > (T(end) - 0.5))
                BoutOffsets = [offsets(Temp > 0.5); (offsets(end) - 0.5)];
            else
                BoutOffsets = [offsets(Temp > 0.5); offsets(end)];
            end
        else
            BoutOnsets = [(onsets(1) + 0.5); onsets(find(Temp > 0.5) + 1)];
            if (offsets(end) > (T(end) - 0.5))
                BoutOffsets = [offsets(Temp > 0.5); (offsets(end) - 0.5)];
            else
                BoutOffsets = [offsets(Temp > 0.5); offsets(end)];
  ...
  %          end
            ...
            for BoutNo = 1:size(Bouts,1),
                Bout.BoutPSD{BoutIndex} = fft(Power(find((SongTime >= Bouts(BoutNo,1)) & (SongTime <= Bouts(BoutNo,2)))), 64000)';
