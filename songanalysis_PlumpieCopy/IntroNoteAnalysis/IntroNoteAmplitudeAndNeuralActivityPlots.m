function [] = IntroNoteAmplitudeAndNeuralActivityPlots(INoteFile, AlignmentPoint, PreTime, PostTime, ColorScale)
...
%    Notes = load([NoteFileDir, '/', ANotes.FileName{i}, '.not.mat']);
    NeuronTime = 0:IFRdt:ANotes.FileLength{i};
    IFR = CalculateIFR(ASpikeData{i}, NeuronTime);
    IFR = conv(IFR, GaussWin, 'same');
    ...
    if (~isempty(MotifIntroNoteIndices{i}))
        ...
        for k = 1:length(UniqueBouts),
            ...
            if (MotifIntroNoteIndices{i}.Indices{BoutIndex}(1) == BoutIndices{i}((UniqueBouts(k)-1)*2 + 1))
                ...
                NeuronActivity{TotalBouts} = [(NeuronTime((find(NeuronTime > StartTime, 1, 'first')):(find(NeuronTime < EndTime, 1, 'last'))) - AlignmentTime); IFR((find(NeuronTime > StartTime, 1, 'first')):(find(NeuronTime < EndTime, 1, 'last')))];
...
AllNeuronActivity = zeros(length(Amplitudes), max(Lengths));
...
for i = 1:length(SortedIndices),
    ...
    AllNeuronActivity(i,:) = interp1(NeuronActivity{SortedIndices(i)}(1,:), NeuronActivity{SortedIndices(i)}(2,:), AmpTime);
end
figure
subplot(2,1,1);
imagesc(AmpTime, [1:1:size(AllNeuronActivity,1)], imcomplement(AllNeuronActivity));
set(gcf, 'Color', 'k');
colormap(ColorScale);
...
colorbar;
figure;
imagesc(AmpTime, [1:1:2*size(AllAmps,1)], imcomplement([AllNeuronActivity/max(max(AllNeuronActivity)); zeros(2, size(AllAmps,2)); AllAmps/max(max(AllAmps))]));
set(gcf, 'Color', 'k');
colormap(ColorScale);
set(gca, 'XColor', 'w', 'YColor', 'w');
xlabel('Time from the beginning of motif (sec)', 'FontSize', 16, 'FontWeight', 'bold', 'Color', 'w');
ylabel('Bout #', 'FontSize', 16, 'FontWeight', 'bold', 'Color', 'w');
set(gca, 'YTick', [(1:20:size(AllAmps,1)) ((size(AllAmps,1):20:size(AllAmps,1)*2)+3)], 'YTickLabel', [(1:20:size(AllAmps,1)) (1:20:size(AllAmps,1))]);
set(gca, 'FontSize', 14, 'FontWeight', 'bold', 'Color', 'w');
colorbar;
figure;
plot(AmpTime, mean(AllAmps/max(max(AllAmps))), 'b');
hold on;
plot(AmpTime, mean(AllNeuronActivity/max(max(AllNeuronActivity))), 'r');
