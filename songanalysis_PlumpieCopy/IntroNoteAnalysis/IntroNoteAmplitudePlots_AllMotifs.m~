function [] = IntroNoteAmplitudePlots_AllMotifs(DataDir, NoteDir, FileList, AlignmentPoint, PreTime, PostTime, ColorScale)
...
%   
    ...
    for j = INLabels,
        smooth(ceil(ANotes.Onsets{i}(j)*NewFs):floor(ANotes.Offsets{i}(j)*NewFs)) = smooth(ceil(ANotes.Onsets{i}(j)*NewFs):floor(ANotes.Offsets{i}(j)*NewFs))*2.5;
    end
    
    if (~isempty(MotifIntroNoteIndices{i}))
        for k = 1:length(MotifIntroNoteIndices{i}.MotifOnsets),
            if (MotifIntroNoteIndices{i}.NoofINs{k} > 0)
                TotalBouts = TotalBouts + 1;
                MotifOnset = MotifIntroNoteIndices{i}.MotifOnsets{k};
                if (AlignmentPoint == 0)
                    AlignmentTime = MotifOnset;
                else
                    if (AlignmentPoint == -1000)
                        AlignmentTime = MotifIntroNoteIndices{i}.Onsets{k}(1);
                    end
                end
...
%                 end
                ...
                NoofINs(TotalBouts) = MotifIntroNoteIndices{i}.NoofINs{k};
            end
        end
    end
end
