function [] = FullIntroNoteAnalysisMultiUnit(DataDir, RecFileDir, NoteFileList, NoteFileDir, FileType, ContinuousOrNot, MotifSylls, StartTrans, GaussWidth, SpikeChanNo, OutputPattern)
...
%[SpikeData, ASpikeData] = INASmoothRectifiedActivity(DataDir, RecFileDir, NoteFiles, FileType, GaussWin, ContinuousOrNot, SpikeChanNo);
INASmoothRectifiedActivity(DataDir, RecFileDir, NoteFiles(1), FileType, GaussWin, ContinuousOrNot, SpikeChanNo);
...
% end
% SimValidFiles = ~cellfun(@isempty, MotifSimIndices, 'UniformOutput', 1);
% Labels(~SimValidFiles) = [];
% MotifSimIndices(~SimValidFiles) = [];
% 
