function [] = PlotSeqSAPFeatsFromMatFile(MatFile, Feature, MaxSyllLen)
...
TempFields = fieldnames(DataFiles{i}.DirBout.Syll);
if (strfind(Feature, 'PC'))
    ...
    for i = [MatchIndex 9],
        for Files = 1:length(DataFiles),
            TempFet = [];
            for j = 1:length(DataFiles{Files}.DirBout.Syll.FileName),
                TempFetVal = eval(['DataFiles{Files}.DirBout.Syll.',TempFields{i},'{j}']);
                TempFet = [TempFet; TempFetVal];
            end
            DirFeats{Files}{ColIndex} = TempFet;

            TempFet = [];
            for j = 1:length(DataFiles{Files}.UnDirBout.Syll.FileName),
                TempFetVal = eval(['DataFiles{Files}.UnDirBout.Syll.',TempFields{i}, '{j}']);
                TempFet = [TempFet; TempFetVal];
            end
    ...
    TempAxis = [];
    figure(FigureIndex);
    annotation('textbox',[0.4 0.9 0.2 0.05], 'FontSize',12,'FontWeight','bold','String',[BirdName, ' - Directed song - ', Feature],'LineStyle','none');
    figure(FigureIndex + 1);
    annotation('textbox',[0.4 0.9 0.2 0.05], 'FontSize',12,'FontWeight','bold','String',[BirdName, ' - Undirected song - ', Feature],'LineStyle','none');
    ...
    for i = 1:length(DirFeats),
        ...
        for Sylls = 1:size(DirFeats{1}{1}, 2),
            plot(DirFeats{i}{2}(:,Sylls), DirFeats{i}{1}(:,Sylls), [Cols(Sylls),'+'], 'MarkerSize', 2);
            hold on;
        end
        axis tight;
        TempAxis(end+1, :) = axis;
        ...
        for Sylls = 1:size(UnDirFeats{1}{1}, 2),
            plot(UnDirFeats{i}{2}(:,Sylls), UnDirFeats{i}{1}(:,Sylls), [Cols(Sylls),'+'], 'MarkerSize', 2);
            hold on;
        end
        axis tight;
        TempAxis(end+1, :) = axis;
