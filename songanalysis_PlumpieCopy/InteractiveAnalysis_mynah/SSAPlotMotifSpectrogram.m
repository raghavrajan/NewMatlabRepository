function  [Spectrogram] = SSAPlotMotifSpectrogram(DirectoryName, RecFileDirectoryName, MedianMotif,  MainFigure, SpectrogramPlot, FileType)

if (strfind(FileType, 'obs')) 
    [RawSong, Fs] = SSASoundIn(DirectoryName, RecFileDirectoryName, [MedianMotif.FileName{1}], 'obs0r');
else
    [RawSong,Fs] = SSAReadOKrankData(DirectoryName, RecFileDirectoryName, [MedianMotif.FileName{1}], 1);
end
...
cm = disp_idx_spect(idx_spect, time_spect, freq_spect, -55, ...
    0, 1.5, 'gray', 'classic');
axis([-0.1 (MedianMotif.Length) 300 10000]);
set(gca,'Visible','off');
