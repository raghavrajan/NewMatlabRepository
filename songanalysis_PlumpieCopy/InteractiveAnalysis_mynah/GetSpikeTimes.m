function [SpikeData, GenParam.ThresholdingParameters] = GetSpikeTimes(GenParam, FileInfo);    

cd(GenParam.RawDataDirectory);
DirectoryName = pwd;
if (ispc)
    DirectoryName(end + 1) = '\';
else
...
if (strfind(GenParam.FileType, 'obs'))
    channel_string = ['obs',num2str(FileInfo.NChans - GenParam.SpikeChanNo),'r'];
end

SpikeData = cell(length(FileInfo.FileNames), 1);

for i = 1:length(FileInfo.FileNames),
    disp(FileInfo.FileNames{i});
    if (strfind(GenParam.FileType, 'obs'))
        [RawData, Fs] = soundin_copy(DirectoryName, FileInfo.FileNames{i},channel_string);
        RawData = RawData * 500/32768; 
    else
        if (strfind(GenParam.FileType, 'okrank'))
            [RawData,Fs] = ReadOKrankData(DirectoryName, FileNames{i},num2str(ChannelNo));
            RawData = RawData * 100;
            cd(PresentDirectory);
        end
    end
