        ...
        for i = Indices,
            ...
            for j = 1:length(INRaster{i})-1,
                ...
                if (~isempty(INRaster{i}{j}))
                    ...
                    PlotRaster(TempRaster, Colour, 2, PlotYOffset, MinTrials);
                end
                               
                fill([zeros(1,MinTrials) flipud(SyllOffsetRaster{i}{j}(1:MinTrials,1))'] + ((j-i)*PlotWidth), [linspace(0.5, MinTrials+0.5, MinTrials) linspace(MinTrials+0.5, 0.5, MinTrials)]+PlotYOffset, 'k', 'EdgeColor', 'none', 'FaceColor', [0.65 0.65 0.65], 'FaceAlpha', 0.5);
