function [RandomOrderSequences] = CalculateRandomOrderingAcousticFeatures(AllINFeatLabels, AllINFeats, AllINFeatNoofINs, ValidIntroNoteNos, NumReps)
...
for RepNo = 1:NumReps,
    ...
    for j = ValidIntroNoteNos',
        ...
        for k = 1:j,
            ...
            TempINFeatLabels = [TempINFeatLabels; [ones(length(INGroupIndices),1)*(k-j-1) ones(length(INGroupIndices),1)*k]];
        end
    end
        ...
        for j = min(TempINFeatLabels(Indices1,1)):1:max(TempINFeatLabels(Indices1,1)),
            ...
            for k = ValidIntroNoteNos',
                ...
                for m = min(TempINFeatLabels(Indices2,1)):1:max(TempINFeatLabels(Indices2,1)),
                    ...
                    if ((j == m) && (i == k))
                        ...
                        Distance = pdist2(mean(TempINFeats(INGroup1,:)), mean(TempINFeats(INGroup2,:)), 'mahalanobis', cov(TempINFeats(INGroup2,:)));
                        if (j == m)
            ...
            if (~isempty(TempSamePosDist) && ~isempty(TempOtherPosDist))
                ...
                if ((length(TempSamePosDist) > 1) && (length(TempOtherPosDist) > 1))
                    RandomOrderSequences.CommonLastIndividualDistances{INNo}(NumReps, :) = [mean(TempSamePosDist) mean(TempOtherPosDist) std(TempSamePosDist) std(TempOtherPosDist) i j];
                else
                    if (length(TempSamePosDist) > 1)
                        RandomOrderSequences.CommonLastIndividualDistances{INNo}(NumReps, :) = [mean(TempSamePosDist) mean(TempOtherPosDist) std(TempSamePosDist) NaN i j];
                    else
                        if (length(TempOtherPosDist) > 1)
                            RandomOrderSequences.CommonLastIndividualDistances{INNo}(NumReps, :) = [mean(TempSamePosDist) mean(TempOtherPosDist) NaN std(TempOtherPosDist) i j];
                        else
                            RandomOrderSequences.CommonLastIndividualDistances{INNo}(NumReps, :) = [mean(TempSamePosDist) mean(TempOtherPosDist) NaN NaN i j];
                        end
                    end
                end
            end
    ...
    for i = ValidIntroNoteNos',
        ...
        for j = min(TempINFeatLabels(Indices1,2)):1:max(TempINFeatLabels(Indices1,2)),
            ...
            if (~isempty(TempSamePosDist) && ~isempty(TempOtherPosDist))
                INNo = INNo + 1;
                if ((length(TempSamePosDist) > 1) && (length(TempOtherPosDist) > 1))
                    RandomOrderSequences.CommonFirstIndividualDistances{INNo}(NumReps, :) = [mean(TempSamePosDist) mean(TempOtherPosDist) std(TempSamePosDist) std(TempOtherPosDist) i j];
                else
                    if (length(TempSamePosDist) > 1)
                        RandomOrderSequences.CommonFirstIndividualDistances{INNo}(NumReps, :) = [mean(TempSamePosDist) mean(TempOtherPosDist) std(TempSamePosDist) NaN i j];
                    else
                        if (length(TempOtherPosDist) > 1)
                            RandomOrderSequences.CommonFirstIndividualDistances{INNo}(NumReps, :) = [mean(TempSamePosDist) mean(TempOtherPosDist) NaN std(TempOtherPosDist) i j];
                        else
                            RandomOrderSequences.CommonFirstIndividualDistances{INNo}(NumReps, :) = [mean(TempSamePosDist) mean(TempOtherPosDist) NaN NaN i j];
                        end
                    end
                end
            end
