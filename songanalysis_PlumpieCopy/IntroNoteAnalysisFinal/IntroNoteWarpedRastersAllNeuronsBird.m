function [PC] = IntroNoteWarpedRastersAllNeuronsBird(Neural_INR, BinSize, PreMotorLatency, MinNumber, MinPlotNumber, INNum, varargin)
...
plot3(Score(Start:End,end), Score(Start:End, end-1), Score(Start:End,end-2), ['k', Symbol, '-']);
...
for i = INNum:-1:1,
    ...
    End = find(NeuronData{1}.RasterPST(INNum).Edges > -End, 1, 'first');
    plot3(Score(Start:End,end), Score(Start:End, end-1), Score(Start:End,end-2), [Colours(mod(i-1, length(Colours))+1), 'o', '-']);
    
    figure(TrajFig(2));
    Start = End;
    End = sum(MedianGapDurs(1:i-1)) + sum(MedianSyllDurs(1:i-1));
    End = find(NeuronData{1}.RasterPST(INNum).Edges > -End, 1, 'first');
    plot3(Score(Start:End,end), Score(Start:End, end-1), Score(Start:End,end-2), [Colours(mod(i-1, length(Colours))+1), '+', '-']);
end

hold on;
Start = End;
plot3(Score(Start:end,end), Score(Start:end, end-1), Score(Start:end,end-2), ['m', Symbol, '-']);
