function [IntroNoteResults] = IntroNoteNeuralAnalysis_MU(DataDir, RecFileDir, SongFileList, NoteFileDir, FileType, MotifSylls, InterBoutInterval, StartSylls, SpikeFileDir, SpikeChanNo, SpikeClusters, ContinuousOrDiscrete, ContinuousFileTime, varargin)

% Get bout details with a function ScreenBouts %
if (nargin > 13)
    ContinuousFiles = varargin{1};
    [BoutDetails, AllBoutDetails] = ScreenBouts_NeuralActivity_MU(DataDir, RecFileDir, FileType, NoteFileDir, SongFileList, InterBoutInterval, MotifSylls, SpikeFileDir, SpikeChanNo, SpikeClusters, ContinuousOrDiscrete, ContinuousFileTime, ContinuousFiles);
else
    [BoutDetails, AllBoutDetails] = ScreenBouts_NeuralActivity_MU(DataDir, RecFileDir, FileType, NoteFileDir, SongFileList, InterBoutInterval, MotifSylls, SpikeFileDir, SpikeChanNo, SpikeClusters, ContinuousOrDiscrete, ContinuousFileTime);
end
...
for i = 1:length(BoutDetails),
    ...
    for j = 1:length(MotifSylls),
        TempMatches = find(BoutDetails(i).labels == MotifSylls(j));
    ...
    for j = 2:length(Matches),
        ...
        if (isempty(IntroNotes))
            IntroNoteResults.WithinBoutNoofINs(INWithinBoutNo,:) = [0 min(MotifIntroSyllsFlag) (BoutDetails(i).onsets(Matches(j)) - BoutDetails(i).offsets(Matches(j)-1))];
        else
            ...
            IntroNoteResults.WithinBoutNoofINs(INWithinBoutNo,:) = [length(IntroNotes) min(MotifIntroSyllsFlag) (BoutDetails(i).onsets(IntroNotes(1)) - BoutDetails(i).offsets(IntroNotes(1)-1))];
...
IntroNoteResults.AllBoutDetails = AllBoutDetails;
