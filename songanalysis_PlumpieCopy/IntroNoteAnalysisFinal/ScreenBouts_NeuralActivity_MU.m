function [BoutDetails, AllBoutDetails] = ScreenBouts_NeuralActivity_MU(RawDataDir, RecFileDir, FileType, NoteFileDir, SongFileList, InterBoutInterval, MotifSylls, SpikeFileDir, SpikeChanNo, SpikeClusters, ContinuousOrDiscrete, varargin)
    ...
    for FileNo = 1:length(ContinuousFiles),
        ...
        for i = ContinuousFiles{FileNo},
            ...
            if (strfind(FileType, 'okrank'))
                ...
                [TempSpikeData, Fs] = SSAReadOKrankData(RawDataDir, RecFileDir, SongFile, SpikeChanNo);
                Song = [Song; TempSong];
                ...
                if (strfind(FileType, 'wav'))
                    ...
                    if (strfind(FileType, 'obs'))
                        ...
                        [TempSpikeData, Fs] = SSASoundIn([RawDataDir, '/'], RecFileDir, SongFile, channel_string);
                        % Convert to V - 5V on the data acquisition is 32768
                        TempSong = TempSong * 5/32768;
                        Song = [Song; TempSong];
                        
                        TempSpikeData = TempSpikeData * 5/32768;
                        SpikeData = [SpikeData; TempSpikeData];
                    end
                end
            end
        ...
        if (FileNo == 1)
            Threshold = mean(SpikeData(1:round(5*Fs))) + 3 * std(SpikeData(1:round(5*Fs)));
            figure;
            plot((1:1:length(Song))/Fs, SpikeData);
            hold on;
            plot([1/Fs length(Song)/Fs], [Threshold Threshold], 'r--');
            title(SongFileList);
            axis tight;
            saveas(gcf, ['/home/raghav/HVCPlots/', SongFileList, '.MUActivity.tiff'], 'tiff');            
        end

        Time = (1:1:length(Song))/Fs;
        [pks, locs] = findpeaks(SpikeData, 'MINPEAKHEIGHT', Threshold);
        ActualSpikeTimes = Time(locs);       
...
%         smooth = spline(T, smooth, Time);
    ...
    for i = 1:length(NoteFiles),
        ...
        if (strfind(FileType, 'okrank'))
            ...
            [SpikeData, Fs] = SSAReadOKrankData(RawDataDir, RecFileDir, SongFile, SpikeChanNo);
        else
            if (strfind(FileType, 'wav'))
                cd(RawDataDir);
                ...
                if (strfind(FileType, 'obs'))
                    ...
                    channel_string = strcat('obs',num2str(SpikeChanNo),'r');
                    [SpikeData, Fs] = SSASoundIn([RawDataDir, '/'], RecFileDir, SongFile, channel_string);

                    % Convert to V - 5V on the data acquisition is 32768
                    Song = Song * 5/32768;
                    SpikeData = SpikeData * 5/32768;
                end
            end
        end
