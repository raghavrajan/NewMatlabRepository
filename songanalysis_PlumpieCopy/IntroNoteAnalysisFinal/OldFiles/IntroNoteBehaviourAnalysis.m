function [IntroNoteResults] = IntroNoteBehaviourAnalysis(DataDir, RecFileDir, SongFileList, NoteFileDir, FileType, MotifSylls, InterBoutInterval, StartSylls)

% Get bout details with a function ScreenBouts %
BoutDetails = ScreenBouts(DataDir, RecFileDir, FileType, NoteFileDir, SongFileList, InterBoutInterval, MotifSylls);
...
for i = 1:length(BoutDetails),
    ...
    for j = 2:length(Matches),
        ...
        if (isempty(IntroNotes))
            IntroNoteResults.WithinBoutNoofINs(INWithinBoutNo,:) = [0 min(MotifIntroSyllsFlag) (BoutDetails(i).onsets(Matches(j)) - BoutDetails(i).offsets(LastMotifSyll))];
        else
            ...
            IntroNoteResults.WithinBoutNoofINs(INWithinBoutNo,:) = [length(IntroNotes) min(MotifIntroSyllsFlag) (BoutDetails(i).onsets(IntroNotes(1)) - BoutDetails(i).offsets(LastMotifSyll))];
