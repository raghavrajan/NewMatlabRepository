function [BoutDetails, AllBoutDetails] = DIR_ScreenBouts(RawDataDir, RecFileDir, FileType, NoteFileDir, SongFileList, InterBoutInterval, MotifSylls, ContinuousOrDiscrete, varargin)
...
%             end
            ...
            if (Flag == 1)
                ...
                if (Notes.onsets(Bouts(j,1))/1000 > InterBoutInterval)
                    ...
                    AllBoutDetails(AllBouts).BoutOnset = 1/Fs;
                end
                if (Notes.offsets(Bouts(j,2))/1000 < (Time(end) - InterBoutInterval))
                    AllBoutDetails(AllBouts).BoutOffset = Notes.offsets(Bouts(j,2))/1000 + InterBoutInterval;
                else
                    AllBoutDetails(AllBouts).BoutOffset = Time(end);
                end
