function [IntroNoteResults] = IntroNoteBehaviourAnalysis(DataDir, RecFileDir, SongFileList, NoteFileDir, FileType, MotifSylls, InterBoutInterval, StartSylls, ContinuousOrDiscrete, ContinuousFileTime, varargin)
...
for i = 1:length(BoutDetails),
    ...
    if (Matches > 1)
        ...
        IN_Indices = [];
        for IN_Index = 1:length(IntroNoteSylls),
            IN_Indices = [IN_Indices find(BoutDetails(i).labels(1:Matches) == IntroNoteSylls(IN_Index))];
        end
    ...
    for j = 2:length(Matches),
        ...
        for k = Matches(j-1):Matches(j),
            if (isempty(strfind([MotifSylls, IntroNoteSylls], BoutDetails(i).labels(k))))
        ...
        IntroNotes = [];
        for IN_Index = 1:length(IntroNoteSylls),
            IntroNotes = [IntroNotes find(BoutDetails(i).labels(Matches(j-1):Matches(j)) == IntroNoteSylls(IN_Index))];
        end
...
INBouts = [];
for i = 1:length(IntroNoteSylls),
    INBouts = [INBouts find(FirstSyll == IntroNoteSylls(i))];
end

IntroNoteResults.MeanIN_INBouts = mean(IntroNoteResults.NoofINs(INBouts));
