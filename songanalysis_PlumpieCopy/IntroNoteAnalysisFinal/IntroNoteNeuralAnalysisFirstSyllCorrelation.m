function [INNeuralAnalysisResults] = IntroNoteNeuralAnalysisFirstSyllCorrelation(Neural_INR, BinSize)
...
for i = 1:length(Neural_INR.NoofINs),
    ...
    for j = 1:Neural_INR.NoofINs(i),
        ...
        INSpikeTimes{TrialIndex}{j} = Neural_INR.BoutDetails(i).SpikeTimes(SpikeIndices) - INStartTime - PreMotorLag;
        ...
        GapSpikeTimes{TrialIndex}{j} = Neural_INR.BoutDetails(i).SpikeTimes(SpikeIndices) - INEndTime - PreMotorLag;
        ...
        INGapSpikeTimes{TrialIndex}{j} = Neural_INR.BoutDetails(i).SpikeTimes(SpikeIndices) - INStartTime - PreMotorLag;
        INGapDur{TrialIndex}{j} = NextSyllStartTime - INStartTime;
        
        IFRINStartIndex = find(Neural_INR.BoutDetails(i).IFR(1,:) <= (INStartTime), 1, 'last');
        IFRINEndIndex = find(Neural_INR.BoutDetails(i).IFR(1,:) <= (INEndTime), 1, 'last');
        IFRINGapEndIndex = find(Neural_INR.BoutDetails(i).IFR(1,:) <= (NextSyllStartTime), 1, 'last');
