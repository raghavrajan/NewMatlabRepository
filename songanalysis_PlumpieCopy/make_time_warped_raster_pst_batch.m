function [SpikeTrain,median_syllable_starting,median_gap_starting] = make_time_warped_raster_pst_batch(DirectoryName,FileName,FileExt,FileNos,HighThresh,LowThresh,ChannelNo,BinSize,Motif,SpikeSortOrThreshold,varargin)
...
for i = 1:length(FileNos),
    ...
    if (strfind(SpikeSortOrThreshold,'spikesort'))
        ...
        for ClusterIndex = 1:length(ClusterNos),
            ...
            [TempSpikeAmplitudes,TempSpikeWaveforms] = GetSpikeAmplitudes(DirectoryName,DataFileName,ChannelNo,TempSpikes);
...
%             The following section is to include outliers
        ...
        threshold_observer_spike_data(DirectoryName,DataFileName,ChannelNo,HighThresh,LowThresh);
    ...
    pathname = pwd;
    pathname = strcat(pathname,'/');
    [TempRawData,Fs] = soundin_copy(pathname,DataFileName,['obs',num2str(ChannelNo),'r']);
