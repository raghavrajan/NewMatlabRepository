function autocorr_ff_pinterp_plotnotes_dir_undir(batch_in,note, ff_low, ff_hi,feature,dir,plot_option) % feature, dir, undir were added by Raghav as arguments
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%       
       index = strfind(notefile,'.cbin');
       if (length(index) == 0)
           index = strfind(notefile,'.not.mat');
       end
       
       FileNum = str2double(notefile((index - 4):(index - 1)));
       if (find(dir == FileNum))
           DirUndir = 'D';
       else
           DirUndir = 'U';
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%       
       intro_notes = findstr('b',labels);
       if (length(intro_notes) > 0)
       else
           intro_notes = findstr('a',labels);
       end
       
       bout_indices = find(diff(intro_notes) > 1);
       bout_begins = [intro_notes(1) intro_notes(bout_indices + 1)];
       
       BoutNo = [];
       for MotifPositionNo = 1:num_matches,
           BoutNo(MotifPositionNo) = find(bout_begins < matches(MotifPositionNo),1,'last');
       end
       BoutNo = BoutNo + MaxBoutNo;
       MaxBoutNo = max(BoutNo);
       
       MotifPositions = [];
       for BoutNos = 1:max(BoutNo),
           MotifPositionIndices = find(BoutNo == BoutNos);
           if (length(MotifPositionIndices) > 0)
               MotifPositionIndices = MotifPositionIndices - MotifPositionIndices(1) + 1; 
               MotifPositions = [MotifPositions MotifPositionIndices];
           end
       end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            %get raw data
             [rawsong,Fs]=soundin_copy('',soundfile,'filt');
%             [rawsong,Fs]=soundin_copy('',rootfile,'obs0r');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%             resample the raw song to 44100KHz
%             rawsong = resample(rawsong,44100,32000);
%             Fs = 44100;
...
%             figure;
%             imagesc((1:size(m_spec_deriv,1)),32000*(1:size(m_spec_deriv,2))/1000/1024,m_spec_deriv') 
%             set(gca,'YDir','normal');
%             colormap(gray);
%             load('parameters.mat');
...
%             mean_FM = mean(m_FM);
%             mean_Entropy = mean(m_Entropy);
%             mean_Amplitude = mean(m_amplitude);
%             mean_AM = mean(m_AM);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%            
            % plot the note
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%            
            if (strmatch('on',plot_option))
                ...
                set(gcf,'Position',[scrsize(3)/4 scrsize(4)/3 [scrsize(3)/2] [2*scrsize(4)/3]])                
...
%                     tmp_plot_function = m_FM/90*250*32000/1024;
...
%                             tmp_plot_function = tmp_plot_function/max(tmp_plot_function)*250* 32000/1024;
%                             plot((1:size(m_spec_deriv,1))/1000,tmp_plot_function,'r');            
%                         end
%                     end
%                 end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% added by Raghav
%             ffreq{k,3} = mean_Pitch;
%             ffreq{k,4} = mean_PitchGoodness;
%             ffreq{k,5} = mean_FM;
%             ffreq{k,6} = mean_Entropy;
%             ffreq{k,7} = mean_Amplitude;
%             ffreq{k,8} = mean_AM;
% 
            ffreq{k,9} = length(syllable)/Fs;
            ffreq{k,10} = SyllableTime;
            ffreq{k,11} = DirUndir;
            ffreq{k,12} = notefile;
            ffreq{k,13} = BoutNo(i);
            ffreq{k,14} = MotifPositions(i);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %ffreq{k,3}=peak;
            k=k+1;
