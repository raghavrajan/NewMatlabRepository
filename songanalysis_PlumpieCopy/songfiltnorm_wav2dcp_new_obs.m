...
Fs = 44100
% this has been changed by Raghav for observer files
...
attenuation = input(['How much do you attenuate the song? :0,-10,-20,-30, -40 (dB)'])
if attenuation == 0
    ATTENtype = 'none'
...
ifile = 0
%while 1
% next 1 lines have been commented out by Raghav
...
% Next line added by Raghav
songname = input('Enter name of variable that has song data: ');

% Next 2 lines commented out by Raghav
%   songfile = fullfile(songbasedir,songname);
%   [song,Fs,nbits]=wavread16(songfile);
    
    % Next 3 lines added by Raghav
    song = songname;
    Fs = 44100;
    nbits = 16;
  ...
  switch lower(NORMTYPE)
    ...
    case 'silencecut_rms'
        % Commented out the next two lines and then added 3 lines - Raghav
%       BGfile = fullfile(BGbasedir,BGnoisefile);
%       [BGnoise,Fs,nbits]=wavread16(BGfile); 
        BGnoise = BG;
        Fs = 44100;
        nbits = 16;
  ...
  if Fsw ~= Fs
	...
	eval(resampcommand)
	[normsong,Fsr,nbits] = wavread16(resampsongfile);
	if Fsr ~= Fsw
  ...
  switch lower(ATTENtype)
    case 'none'
       if songtype == 2
       normdcpsongfile = [outputfile, '.40k.rms.dcp']
       else
    ...
    case 'minus10'
       if songtype == 2
       normdcpsongfile = [outputfile, '-10dB.40k.rms.dcp']
       else
    ...
    case 'minus20'
       if songtype == 2
       normdcpsongfile = [outputfile, '-20dB.40k.rms.dcp']
       else
    ...
    case 'minus30'  
       if songtype == 2
       normdcpsongfile = [outputfile, '-30dB.40k.rms.dcp']
       else
    ...
    case 'minus40'
       if songtype == 2
       normdcpsongfile = [outputfile, '-40dB.40k.rms.dcp']
       else
