function [BoutStats] = AnalyseIntroNotes2(RawDataDir, FileType, NoteDirName, NoteFiles, Motif, Motif2, IntroSyllables, ContinuousOrNot)
        ...
        for i = 0:length(Bouts),
            ...
            if (~isempty(Motifs))
                ...
                if ((Notes.onsets(StartIndex) > InterBoutInterval) && (Notes.offsets(EndIndex) < (Time(end) - InterBoutInterval)))
                    BoutStats.Labels = [BoutStats.Labels Notes.labels(StartIndex:EndIndex) 'q'];
                    BoutStats.IntroNoteNos(BoutNo) = 0;
                    for j = 1:length(IntroSyllables),
