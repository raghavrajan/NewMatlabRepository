function [BoutPosition_SyllDistribution] = Prasanth_PlotBoutPosSyllDistribution(BirdParameters, MinTrialNo)
...
for i = 1:length(BirdParameters),
    ...
    for Reps = 1:10000,
        ...
        for j = 1:size(RandomBoutPos_SyllDurations,2),
            ...
            Random_SyllDurHistCorr(Reps, j) = corr(BoutPosition_SyllDistribution{i}(end,:)', RandomBoutPosition_SyllDistribution(j,:)');
        end
    end
    ...
    for j = Indices(:)',
        if ((nanmean(BoutPosition_SyllDurations{i}(:,j)) > prctile(RandomMeanBoutPos_SyllDurations(:,j), 97.5)) || (nanmean(BoutPosition_SyllDurations{i}(:,j)) < prctile(RandomMeanBoutPos_SyllDurations(:,j), 2.5)))
            ...
            SigIndices(j) = 1;
            if (nanmean(BoutPosition_SyllDurations{i}(:,j)) > prctile(RandomMeanBoutPos_SyllDurations(:,j), 97.5))
                DurSigIndices{i}(j) = 1;
            else
                if (nanmean(BoutPosition_SyllDurations{i}(:,j)) < prctile(RandomMeanBoutPos_SyllDurations(:,j), 2.5))
                    DurSigIndices{i}(j) = -1;
                end
            end
        else
    ...
    for j = Indices(:)',
        ...
        SyllDurHistCorr{i}(j) = corr(BoutPosition_SyllDistribution{i}(end,:)', BoutPosition_SyllDistribution{i}(j,:)');
        if ((SyllDurHistCorr{i}(j) > prctile(Random_SyllDurHistCorr(:,j), 97.5)) || (SyllDurHistCorr{i}(j) < prctile(Random_SyllDurHistCorr(:,j), 2.5)))
            CorrSigIndices(j) = 1;
            if (SyllDurHistCorr{i}(j) > prctile(Random_SyllDurHistCorr(:,j), 97.5))
                SyllDurHistCorrSigIndices{i}(j) = 1;
            else
                if (SyllDurHistCorr{i}(j) < prctile(Random_SyllDurHistCorr(:,j), 2.5))
                    SyllDurHistCorrSigIndices{i}(j) = -1;
                end
            end
        else
            SyllDurHistCorrSigIndices{i}(j) = 0;
        end
    end
    CorrFigure(i) = figure;
    set(gcf, 'Color', 'w');
...
% Plot color plots for the significance with each row being a bird

LesionOrNormal = inputdlg('Input 1 if this is lesion data and 0 if this is normal data', 'Lesion or Normal Choice box');
LesionOrNormal = str2double(LesionOrNormal{1});


AllBirdDurSigIndices = ones(length(BirdParameters), max(cellfun(@length, DurSigIndices))) * NaN;
for i = 1:length(BirdParameters),
    AllBirdDurSigIndices(i,1:length(DurSigIndices{i})) = DurSigIndices{i};
end
...
hold on;
for i = 1:size(AllBirdDurSigIndices,1),
    imagesc(1:1:length(DurSigIndices{i}), i, AllBirdDurSigIndices(i,1:length(DurSigIndices{i})));
end
...
ylabel('Bird #');
title(['Position in the bout vs. duration significance (n=', num2str(size(AllBirdDurSigIndices,1)), ') birds']);
set(gcf, 'PaperPositionMode', 'auto');
if (LesionOrNormal == 0)
    print(['NormalBirds.AllBirds.BoutPos.vs.DurChangeSignificance.', num2str(BirdParameters(1).Interboutinterval), '.png'], '-dpng', '-r300');
else
    print(['LesionBirds.AllBirds.BoutPos.vs.DurChangeSignificance.', num2str(BirdParameters(1).Interboutinterval), '.png'], '-dpng', '-r300');
end
% Now for syll duration histogram correlation

AllBirdSyllDurHistCorrSigIndices = ones(length(BirdParameters), max(cellfun(@length, SyllDurHistCorrSigIndices))) * NaN;
for i = 1:length(BirdParameters),
    AllBirdSyllDurHistCorrSigIndices(i,1:length(SyllDurHistCorrSigIndices{i})) = SyllDurHistCorrSigIndices{i};
end
...
set(gcf, 'Position', [163 530 1000 400]);
hold on;
for i = 1:size(AllBirdSyllDurHistCorrSigIndices,1),
    imagesc(1:1:length(SyllDurHistCorrSigIndices{i}), i, AllBirdSyllDurHistCorrSigIndices(i,1:length(SyllDurHistCorrSigIndices{i})));
end
colormap([0 0 1; 0.9 0.9 0.5; 1 0 0]);
colorbar('Ticks', [-0.667 0 0.667], 'TickLabels', {'< chance', '= chance', '> chance'});
axis tight;
xlabel('Position within the bout');
ylabel('Bird #');
title(['Position in the bout vs. syll duration histogram correlation significance (n=', num2str(size(AllBirdSyllDurHistCorrSigIndices,1)), ') birds']);
set(gcf, 'PaperPositionMode', 'auto');
if (LesionOrNormal == 0)
    print(['NormalBirds.AllBirds.BoutPos.vs.SyllDurHistCorrChangeSignificance.', num2str(BirdParameters(1).Interboutinterval), '.png'], '-dpng', '-r300');
else
    print(['LesionBirds.AllBirds.BoutPos.vs.SyllDurHistCorrChangeSignificance.', num2str(BirdParameters(1).Interboutinterval), '.png'], '-dpng', '-r300');
end
% First duration
