...
if (isfield(DirFileInfo.UWEventParameters,'EventTimes') > 0)
    EventPlotWidth = (0.8 - 0.02*(length(DirFileInfo.UWEventParameters.EventTimes) - 1))/length(DirFileInfo.UWEventParameters.EventTimes);
    NoofEvents = length(DirFileInfo.UWEventParameters.EventTimes);
else
    if (isfield(UnDirFileInfo.UWEventParameters,'EventTimes') > 0)
        NoofEvents = length(UnDirFileInfo.UWEventParameters.EventTimes);
        EventPlotWidth = (0.8 - 0.02*(length(DirFileInfo.UWEventParameters.EventTimes) - 1))/length(UnDirFileInfo.UWEventParameters.EventTimes);
    else
        return;
    end
end
...
for EventCounter = 1:NoofEvents,
    ...
    if (length(DirFileInfo.RecordLengths) > 0)
        TempEventTime = DirFileInfo.UWEventParameters.EventTimes(EventCounter);
    ...
    if (length(UnDirFileInfo.RecordLengths) > 0)
        TempEventTime = UnDirFileInfo.UWEventParameters.EventTimes(EventCounter);
       for i = 1:2,
