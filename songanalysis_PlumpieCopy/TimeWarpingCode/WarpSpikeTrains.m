function [Raster, PST, Edges, WarpedSpikeTrain, UnWarpedSpikeTrain, SpikeWaveforms] = WarpSpikeTrains(FileInfo, Motif, MedianMotif, BinSize, Latency)
...
for SongNo = 1:length(Songs),
    ...
    TempSpikeAmplitudes = [];
    TempSpikeWaveforms = [];
    OnsetSpikes = FileInfo.SpikeData.Times(find((FileInfo.SpikeData.Times > (FileInfo.Syllables.Start(SongNo,1) - 0.1)) & (FileInfo.SpikeData.Times < FileInfo.Syllables.Start(SongNo,1))));
    OnsetSpikeAmplitudes = FileInfo.SpikeData.Amplitudes(find((FileInfo.SpikeData.Times > (FileInfo.Syllables.Start(SongNo,1) - 0.1)) & (FileInfo.SpikeData.Times < FileInfo.Syllables.Start(SongNo,1))));
    OnsetSpikeWaveforms = FileInfo.SpikeData.Waveforms(find((FileInfo.SpikeData.Times > (FileInfo.Syllables.Start(SongNo,1) - 0.1)) & (FileInfo.SpikeData.Times < FileInfo.Syllables.Start(SongNo,1))),:);    
    if (length(OnsetSpikes > 0))
        TempUnWarpedSpikes = [TempUnWarpedSpikes; (OnsetSpikes - FileInfo.Syllables.Start(SongNo,1))];
    ...
    for Syllable = 1:length(Motif),
        ...
        if (length(SyllableSpikes > 0))
            TempUnWarpedSpikes = [TempUnWarpedSpikes; (SyllableSpikes - FileInfo.Syllables.Start(SongNo,1))];
        ...
        if (Syllable ~=length(Motif))
            ...
            if (length(GapSpikes > 0))
                TempUnWarpedSpikes = [TempUnWarpedSpikes; (GapSpikes - FileInfo.Syllables.Start(SongNo,1))];                
...
%        WarpedSpikeTrain{SongNo} = TempSpikes(find((TempSpikes > (-Latency)) & (TempSpikes < MedianMotif.Length))) + Latency;
        WarpedSpikeTrain{SongNo} = TempSpikes;
        UnWarpedSpikeTrain{SongNo} = TempUnWarpedSpikes;
