function [outvect, adfreq] = SSAread_labv_file_copy(RecFileDirectory, filename, channel, orient)
...
base = filename((q(length(q)) + 1):p(length(p)));
if (ispc)
    recfile = [RecFileDirectory, '\', base 'rec'];
else
    recfile = [RecFileDirectory, '/', base 'rec'];
end
...
%end
if (~ischar(line) | ~strncmp(line,'File create', 11)),
  error([filename ' is not a Labview datafile.']);
end
...
while 1,
  ...
  if strncmp(line, 'ADFREQ =', 8),
     adfreq = sscanf(line, 'ADFREQ = %d');
	  if ~isnumeric(adfreq),
        error(['Attempt to read AD freq. in recfile ' recfile ' failed.']);
	  end
...
fclose(fid);
if (count ~= nscans),
   error('Number of data points read does not match the # of samples in recfile.');
end
