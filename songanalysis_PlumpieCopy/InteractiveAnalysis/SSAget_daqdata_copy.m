function [daqdata, ad_freq] = SSAget_daqdata_copy(RecFileDirectory, filename, chan_spec)
% [daqdata, ad_freq] = get_daqdata(filename, chan_spec)
%
% GET_DAQDATA  Return one channel of data acquisition data from an msong,
...
if strcmp(ftype, 'sun'),
   ...
   [daqdata, ad_freq] = read_rawfile(filename, 'short');

elseif strcmp(ftype, 'lview'),

   [daqdata, ad_freq] = SSAread_labv_file_copy(RecFileDirectory, filename, ch_num, direction);
