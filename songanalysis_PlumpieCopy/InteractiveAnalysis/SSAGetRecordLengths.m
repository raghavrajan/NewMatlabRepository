function [RecordLengths, NChans, StatusString] = SSAGetRecordLengths(DirectoryName, RecFileDirectoryName, FileNames, FileType, ContextString)

StatusString{1} = ContextString;
cd(DirectoryName);
...
for i = 1:length(FileNames),
    if (strfind(FileType, 'observer'))
        ...
        if (ispc)
            RecFid = fopen([RecFileDirectoryName, '\', RecFileName], 'r');
        else
            RecFid = fopen([RecFileDirectoryName, '/', RecFileName], 'r');
        end
        index = 0;
    ...
    StatusString{i + 1} = [FileNames{i}, ': ', num2str(RecordLengths{i}), 's and ', num2str(NChans{i}), ' channels'];
end