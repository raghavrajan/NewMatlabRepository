function [] = SSAAnalyseBoutBeginningandEnds(RawDataDir, RecFileDir, PreBoutDuration, PostBoutDuration, FileInfo, TitleString, Motif, Motif2, FileType)
    ...
    if (~isempty(Motifs))
        if (Motifs(1) == 1)
            ...
            if (~isempty(IntroNoteIndices))
                ...
                MotifOnsetTime = Notes.onsets(IntroNoteIndices(1));
                ...
                if (IntroNoteIndices(1) == 1)
                    ...
                    OnsetIndices = find(Notes.offsets < MotifOnsetTime, 1, 'last');
                    OnsetsRaster = [OnsetsRaster; [(Notes.offsets(OnsetIndices) - MotifOnsetTime) MotifNo]];
                end
                MotifNo = MotifNo + 1;
            else
                NoofINs(MotifNo) = 0;
...
axis(Temp);
title([TitleString, '- Ordered by # of intro notes Onset of first Intro note at time 0'], 'FontSize', 14, 'FontWeight', 'bold');
...
save([FileInfo.FileNames{1}, '.bout_beginnings.mat'], 'MotifRaster', 'OnsetsRaster', 'MotifPST', 'NoofINs', 'BoutInterval');
