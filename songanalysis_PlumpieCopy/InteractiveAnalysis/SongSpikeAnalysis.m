...
handles.SSA.SeqSyll = get(handles.SeqSyllEdit, 'String');
...
[FileNames] = uigetfile({['*'], 'All Files (*.*)'}, 'Choose the directed song files - Use <Shift> or <Control> to select multiple files', 'MultiSelect', 'on');
if (~iscell(FileNames))
    if (strfind(FileNames, 'txt'))
        handles.SSA.DirFileInfo.SongFileList = FileNames;
        Fid = fopen(FileNames, 'r');
        TempFiles = textscan(Fid, '%s', 'DeLimiter', '\n');
        fclose(Fid);
        handles.SSA.DirFileInfo.FileNames = TempFiles{1};
    else
        handles.SSA.DirFileInfo.FileNames{1} = FileNames;
    end
else
    handles.SSA.DirFileInfo.FileNames = FileNames;
end
...
% handles    structure with handles and user data (see GUIDATA)
cd(handles.SSA.RawDataDirectory);
[FileNames] = uigetfile({['*'], 'All Files (*.*)'}, 'Choose either a complete filelist (*.txt) or individual Undirected song files - Use <Shift> or <Control> to select multiple files', 'MultiSelect', 'on');

if (~iscell(FileNames))
    if (strfind(FileNames, 'txt'))
        handles.SSA.UnDirFileInfo.SongFileList = FileNames;
        Fid = fopen(FileNames, 'r');
        TempFiles = textscan(Fid, '%s', 'DeLimiter', '\n');
        fclose(Fid);
        handles.SSA.UnDirFileInfo.FileNames = TempFiles{1};
    else
        handles.SSA.UnDirFileInfo.FileNames{1} = FileNames;
    end
else
    handles.SSA.UnDirFileInfo.FileNames = FileNames;
end
...
ClusterstoLoad(find(ClusterstoLoad == '[')) = [];
ClusterstoLoad(find(ClusterstoLoad == ']')) = [];
if (length(ClusterstoLoad) > 0)
    BlankIndices = find(ClusterstoLoad == ' ');
    NoOfClusters = length(BlankIndices) + 1;
    handles.SSA.ClusterstoLoad = [];
    if (length(BlankIndices) > 0)
        handles.SSA.ClusterstoLoad = [handles.SSA.ClusterstoLoad; str2double(ClusterstoLoad(1:(BlankIndices(1)-1)))];
        for i = 2:length(BlankIndices),
            handles.SSA.ClusterstoLoad = [handles.SSA.ClusterstoLoad; str2double(ClusterstoLoad((BlankIndices(i-1)+1):(BlankIndices(i) - 1)))];
        end
        handles.SSA.ClusterstoLoad = [handles.SSA.ClusterstoLoad; str2double(ClusterstoLoad((BlankIndices(end)+1):end))];
    else
        handles.SSA.ClusterstoLoad = str2double(ClusterstoLoad);
    end
end
disp(handles.SSA.ClusterstoLoad);
guidata(handles.SSAMainWindow, handles);
        ...
        if (handles.SSA.DirSong == 1)
            for i = 1:length(handles.SSA.DirFileInfo.FileNames),
                ...
                if (strfind(handles.SSA.FileType, 'observer'))
                    DotIndex = find(handles.SSA.DirFileInfo.FileNames{i} == '.');
                    if (ispc)
                        SpikeTimes = load([handles.SSA.SpikeFileDir, '\', 'Chan', num2str(handles.SSA.SpikeChanNo), '_', handles.SSA.DirFileInfo.FileNames{i}(1:DotIndex(end)), 'spk']);
                    else
%                        SpikeTimes = load([handles.SSA.SpikeFileDir, '/','Chan', num2str(handles.SSA.SpikeChanNo), '_', handles.SSA.DirFileInfo.FileNames{i}(1:DotIndex(end)), 'spk']);
                        SpikeTimes = load([handles.SSA.SpikeFileDir, '/','Chan', num2str(handles.SSA.SpikeChanNo), '_', handles.SSA.DirFileInfo.FileNames{i}, '.spk']);
                    end
                    SpikeIndices = [];
                    if (SpikeTimes(end,2) > handles.SSA.DirFileInfo.RecordLengths{i})
                        SpikeTimes(:,2) = SpikeTimes(:,2)/1000;
        ...
        if (handles.SSA.UnDirSong == 1)
            for i = 1:length(handles.SSA.UnDirFileInfo.FileNames),
                ...
                if (strfind(handles.SSA.FileType, 'observer'))
                    DotIndex = find(handles.SSA.UnDirFileInfo.FileNames{i} == '.');
                    if (ispc)
                        SpikeTimes = load([handles.SSA.SpikeFileDir, '\','Chan', num2str(handles.SSA.SpikeChanNo), '_', handles.SSA.UnDirFileInfo.FileNames{i}(1:DotIndex(end)), 'spk']);
                    else
%                        SpikeTimes = load([handles.SSA.SpikeFileDir, '/','Chan', num2str(handles.SSA.SpikeChanNo), '_', handles.SSA.UnDirFileInfo.FileNames{i}(1:DotIndex(end)), 'spk']);
                        SpikeTimes = load([handles.SSA.SpikeFileDir, '/','Chan', num2str(handles.SSA.SpikeChanNo), '_', handles.SSA.UnDirFileInfo.FileNames{i}, '.spk']);                        
                    end
                    if (SpikeTimes(end,2) > handles.SSA.UnDirFileInfo.RecordLengths{i})
                        SpikeTimes(:,2) = SpikeTimes(:,2)/1000;
                    end
...
if ((handles.SSA.DirSong == 1))
    ...
    [handles.SSA.DirFileInfo.CorrelationSameSize] = SSACalculateCorrGaussSameSize(handles.SSA.DirFileInfo.WSpikeTrain, handles.SSA.MedianMotif, handles.SSA.GaussianWidth/1000, handles.SSA.PreSongStartDuration/1000, handles.SSA.PreSongEndDuration/1000, 'Directed', handles.SSA.GaussianLen);
...
if ((handles.SSA.UnDirSong == 1))
    ...
    [handles.SSA.UnDirFileInfo.CorrelationSameSize] = SSACalculateCorrGaussSameSize(handles.SSA.UnDirFileInfo.WSpikeTrain, handles.SSA.MedianMotif, handles.SSA.GaussianWidth/1000, handles.SSA.PreSongStartDuration/1000, handles.SSA.PreSongEndDuration/1000, 'Undirected', handles.SSA.GaussianLen);
...
disp(['Saved data to ', OutputFileName]);
...
function SeqSyllEdit_Callback(hObject, eventdata, handles)
% hObject    handle to SeqSyllEdit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of SeqSyllEdit as text
%        str2double(get(hObject,'String')) returns contents of SeqSyllEdit as a double
handles.SSA.SeqSyll = get(hObject, 'String');
guidata(handles.SSAMainWindow, handles);

% --- Executes during object creation, after setting all properties.
function SeqSyllEdit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to SeqSyllEdit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in AnalyseSeqButton.
function AnalyseSeqButton_Callback(hObject, eventdata, handles)
% hObject    handle to AnalyseSeqButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

SlashIndex = find((handles.SSA.RawDataDirectory == '\') | (handles.SSA.RawDataDirectory == '/'));

if (handles.SSA.UnDirSong == 1)
    TitleString = [handles.SSA.UnDirFileInfo.FileNames{1}(1:6), '/', handles.SSA.RawDataDirectory((SlashIndex(end)+1):end)];
else
    TitleString = [handles.SSA.DirFileInfo.FileNames{1}(1:6), '/', handles.SSA.RawDataDirectory((SlashIndex(end)+1):end)];
end

if (handles.SSA.DirSong == 1)
    SSAAnalyseSeq(handles.SSA.SeqSyll, handles.SSA.PSTBinSize/1000, handles.SSA.DirFileInfo, [TitleString, '-Directed song']);
    SSAAnalyseConvSeq(handles.SSA.SeqSyll, handles.SSA.PSTBinSize/1000, handles.SSA.DirFileInfo, [TitleString, '-Directed song']);
    if (strfind(handles.SSA.SeqSyll, 'i'))
        SSAAnalyseIntroNotes(handles.SSA.RawDataDirectory, handles.SSA.RecFileDirectory, handles.SSA.PreBoutDuration, handles.SSA.PostBoutDuration, handles.SSA.DirFileInfo, [TitleString, '-Directed song'], handles.SSA.Motif1, handles.SSA.Motif2, handles.SSA.FileType);
    else
        %SSAAnalyseSeqMotif(handles.SSA.SeqSyll, handles.SSA.PSTBinSize/1000, handles.SSA.DirFileInfo, [TitleString, '-Directed song'], handles.SSA.Motif);
    end
end
...
% --- Executes on button press in AnalyseFeaturesButton.
function AnalyseFeaturesButton_Callback(hObject, eventdata, handles)
% hObject    handle to AnalyseFeaturesButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
SSAAnalyseFeatures(handles.SSA.DirFileInfo);
SSAAnalyseFeatures(handles.SSA.UnDirFileInfo);



function PostBoutDurationEdit_Callback(hObject, eventdata, handles)
% hObject    handle to PostBoutDurationEdit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of PostBoutDurationEdit as text
%        str2double(get(hObject,'String')) returns contents of PostBoutDurationEdit as a double
handles.SSA.PostBoutDuration = str2double(get(hObject, 'String'));
guidata(handles.SSAMainWindow, handles);

% --- Executes during object creation, after setting all properties.
function PostBoutDurationEdit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to PostBoutDurationEdit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function PreBoutDurationEdit_Callback(hObject, eventdata, handles)
% hObject    handle to PreBoutDurationEdit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of PreBoutDurationEdit as text
%        str2double(get(hObject,'String')) returns contents of PreBoutDurationEdit as a double
handles.SSA.PreBoutDuration = str2double(get(hObject, 'String'));
guidata(handles.SSAMainWindow, handles);

% --- Executes during object creation, after setting all properties.
function PreBoutDurationEdit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to PreBoutDurationEdit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in AnalyseMotifTransitionsButton.
function AnalyseMotifTransitionsButton_Callback(hObject, eventdata, handles)
% hObject    handle to AnalyseMotifTransitionsButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if (handles.SSA.DirSong == 1)
    SSAAnalyseMotifTransitions(handles.SSA.RawDataDirectory, handles.SSA.RecFileDirectory, handles.SSA.PreBoutDuration, handles.SSA.PostBoutDuration, handles.SSA.DirFileInfo, [TitleString, '-Directed song'], handles.SSA.Motif1, handles.SSA.Motif2, handles.SSA.FileType);
    SSAAnalyseBoutBeginningandEnds(handles.SSA.RawDataDirectory, handles.SSA.RecFileDirectory, handles.SSA.PreBoutDuration, handles.SSA.PostBoutDuration, handles.SSA.DirFileInfo, [TitleString, '-Directed song'], handles.SSA.Motif1, handles.SSA.Motif2, handles.SSA.FileType);
end

if (handles.SSA.UnDirSong == 1)
    SSAAnalyseMotifTransitions(handles.SSA.RawDataDirectory, handles.SSA.RecFileDirectory, handles.SSA.PreBoutDuration, handles.SSA.PostBoutDuration, handles.SSA.UnDirFileInfo, [TitleString, '-Undirected song'], handles.SSA.Motif1, handles.SSA.Motif2, handles.SSA.FileType);
    SSAAnalyseBoutBeginningandEnds(handles.SSA.RawDataDirectory, handles.SSA.RecFileDirectory, handles.SSA.PreBoutDuration, handles.SSA.PostBoutDuration, handles.SSA.UnDirFileInfo, [TitleString, '-UnDirected song'], handles.SSA.Motif1, handles.SSA.Motif2, handles.SSA.FileType);
end



function Motif1Edit_Callback(hObject, eventdata, handles)
% hObject    handle to Motif1Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of Motif1Edit as text
%        str2double(get(hObject,'String')) returns contents of Motif1Edit as a double
handles.SSA.Motif1 = get(hObject, 'String');
guidata(handles.SSAMainWindow, handles);

% --- Executes during object creation, after setting all properties.
function Motif1Edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to Motif1Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function Motif2Edit_Callback(hObject, eventdata, handles)
% hObject    handle to Motif2Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of Motif2Edit as text
%        str2double(get(hObject,'String')) returns contents of Motif2Edit as a double
handles.SSA.Motif2 = get(hObject, 'String');
guidata(handles.SSAMainWindow, handles);

% --- Executes during object creation, after setting all properties.
function Motif2Edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to Motif2Edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in AnalyseBoutTransitionsButton.
function AnalyseBoutTransitionsButton_Callback(hObject, eventdata, handles)
% hObject    handle to AnalyseBoutTransitionsButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if (handles.SSA.DirSong == 1)
    SSAAnalyseBoutTransitions(handles.SSA.RawDataDirectory, handles.SSA.RecFileDirectory, handles.SSA.PreBoutDuration, handles.SSA.PostBoutDuration, handles.SSA.DirFileInfo, [TitleString, '-Directed song'], handles.SSA.Motif1, handles.SSA.Motif2, handles.SSA.FileType, handles.SSA.ContinuousOrNot);
end

if (handles.SSA.UnDirSong == 1)
    SSAAnalyseBoutTransitions(handles.SSA.RawDataDirectory, handles.SSA.RecFileDirectory, handles.SSA.PreBoutDuration, handles.SSA.PostBoutDuration, handles.SSA.UnDirFileInfo, [TitleString, '-Undirected song'], handles.SSA.Motif1, handles.SSA.Motif2, handles.SSA.FileType, handles.SSA.ContinuousOrNot);
end
...
% Hint: get(hObject,'Value') returns toggle state of ContButton
handles.SSA.ContinuousOrNot = get(hObject, 'Value');
guidata(handles.SSAMainWindow, handles);


% --- Executes on button press in SyllableAnalysisButton.
function SyllableAnalysisButton_Callback(hObject, eventdata, handles)
% hObject    handle to SyllableAnalysisButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

SSASyllableAnalysis(handles)
