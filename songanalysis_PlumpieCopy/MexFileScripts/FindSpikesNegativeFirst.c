  ...
  int nINP, const mxArray *inputs[])
{
      ...
      int length_data, window_size, index, spike_index, i, j, k, waveform_index, min_index, skip_window_size,increment_index,found_spike;
      double lower_threshold, upper_threshold, start_time, time, min_value, sampling_rate;
        ...
        if (data[index] < lower_threshold)
        {
            ...
            for(i = index; i < (index + window_size); i++)
            {
                ...
                if (data[i] > upper_threshold)
                {
                    ...
                    index = min_index;
                    time = start_time + min_index/sampling_rate;
