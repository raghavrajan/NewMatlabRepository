% This was given by Brian, and modified by Satoshi.


% Right now this program is used to convert from 32 kHz dcp files
% to 40kHz dcp files
...
% Will assume input dcp files recorded at this rate!
Fs = 32000
...
%  [song,Fs,nbits]=wavread(songfile);
  ...
  adfreq_header = fgetl(fids)
  songlen = fread(fids,1,'int32')
  adfreq = sscanf(adfreq_header,'AD_FREQ: %d Hz')
  if adfreq ~= 32000
    error('Wrong input dcp file type.')
  end
  [song, sampcount] = fread(fids,songlen,'int16');
  Fs = adfreq
  nbits = 16;
  if sampcount ~= songlen
    error('Could not read all the data!')
  end
  fclose(fids);
  ...
  % Write to file
%  if songtype == 2
    normdcpsongfile = [songfile, '.alx']
%  else
%    normdcpsongfile = [songfile, '.norm.dcp']
%  end
  ...
  numwrite = fwrite(fidw,round(normsong(1:dcpsonglen)),wtype);
