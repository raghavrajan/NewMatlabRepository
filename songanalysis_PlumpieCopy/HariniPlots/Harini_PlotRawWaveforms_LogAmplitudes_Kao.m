function [] = Harini_PlotRawWaveforms_LogAmplitudes_Kao(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)
...
for i = 1:length(IndividualBirds),
    ...
    for j = 1:length(MotifLabels),
        ...
        UnDirectedBouts = IndividualBirds(i).BoutStatistics(find(IndividualBirds(i).BoutStatistics(:,ConditionColumnIndex) == 6), BoutIndexColumnIndex);
        UnDirectedBouts = intersect(UnDirectedBouts, ValidSongBouts);
        
        L0Bouts = IndividualBirds(i).BoutStatistics(find(IndividualBirds(i).BoutStatistics(:,ConditionColumnIndex) == 1), BoutIndexColumnIndex);
        L0Bouts = intersect(L0Bouts, ValidSongBouts);
        
        DirectedL0Bouts = intersect(IndividualBirds(i).BoutStatistics(strmatch('D', IndividualBirds(i).BoutCategorisation, 'exact'), BoutIndexColumnIndex), L0Bouts);
