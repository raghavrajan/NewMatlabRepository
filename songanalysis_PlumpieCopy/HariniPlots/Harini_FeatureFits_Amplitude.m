...
for BirdIndex = 1:length(BirdsToPlot),
    ...
    p(BirdIndex).pack(1, length(UndirBout_Stats{i}{end}.LogAmplitude_SyllLabel));
    
    for SyllIndex = 1:length(UndirBout_Stats{i}{end}.LogAmplitude_SyllLabel),
        DirIndices = find(LogAmplitudeGroupData{i}{SyllIndex}(:,end) == 1);
        UnDirIndices = find((LogAmplitudeGroupData{i}{SyllIndex}(:,end-1) == 6) & (LogAmplitudeGroupData{i}{SyllIndex}(:,end) == 3));
        % First plot the group data
        p(BirdIndex,1,SyllIndex).select();
        hold on;
        ...
        % Now best fit line
        RobustCoeffs = FitCoeffs{i}{SyllIndex};
        
        FixedX_BootStrapCoeffs = BootStrapCI{i}{SyllIndex};
...
%         if (SyllIndex == 2)
%             title(BirdTitles{BirdIndex});
%         end
        if ((SyllIndex == 2) && (BirdIndex == length(BirdsToPlot)))
            xlabel('Distance from the female (cm)');
        end
        
        DistanceXLabelString = [];
        DistanceXValues = [];
        for Dist = 1:length(unique(Distances(LogAmplitudeGroupData{i}{SyllIndex}(DirIndices,end-1)))),
...
for i = UniqueBirdIndices(:)',
    ...
    for j = Syllables(:)',
        ...
        plot(i+rand/10, UnNormalizedSyllableDistanceDirMeans(j,1)/UnNormalizedSyllableUnDirMeans(j), [SymbolColor, Symbols(i)], 'MarkerFaceColor', MarkerSymbolFaceColor);
    end
end
axis tight;
Temp = axis;
...
BirdFinalIndices = [10 11 12 13 5 15 16];
...
for i = L0MoreThanUndirIndices(:)',
    for j = 1:length(FitCoeffs{i}),
        ...
        plot(1.15+rand/10, FitCoeffs{i}{j}(2), [MarkerColorSymbol, Symbols(i)], 'MarkerSize', 4, 'MarkerFaceColor', MarkerSymbolFaceColor);    
    end
end
...
set(gcf, 'PaperPositionMode', 'auto');
print(fullfile(FinalFigureDir, ['HFMicBirds_Example_Summary_Amplitude.eps']), '-depsc2', '-r600');
print(fullfile(FinalFigureDir, ['HFMicBirds_Example_Summary_Amplitude.png']), '-dpng', '-r600');
...
set(gcf, 'PaperPositionMode', 'auto');
print(fullfile(FinalFigureDir, ['HFMicBirds_SyllableWise_Summary_Amplitude.eps']), '-depsc2', '-r600');
print(fullfile(FinalFigureDir, ['HFMicBirds_SyllableWise_Summary_Amplitude.png']), '-dpng', '-r600');
