function [] = Harini_PlotMotifNumber_DirOnlySongs(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)

% ======= Motif number for only the songs classified as DIRECTED ==========
% Plots the number of motifs for only songs classified as directed at each 
% of the locations - averaged over multiple days of experiments
% =========================================================================
...
for i = 1:length(IndividualBirds),
    ...
    for j = 1:length(IndividualBirds(i).Conditions),
        ...
        if (length(DirSongIndices) >= 3)
            plot(0:1:max(IndividualBirds(i).BoutStatistics(:, ColumnIndex)), histc(IndividualBirds(i).BoutStatistics(DirSongIndices, ColumnIndex), 0:1:max(IndividualBirds(i).BoutStatistics(:,ColumnIndex)))/length(DirSongIndices), [Colours(mod(j, length(Colours)) + 1), 'o-'], 'LineWidth', 2);
