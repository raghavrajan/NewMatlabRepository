function [] = HariniPlots_ScriptFile_PaperFigures(CSVTextFile, PlotOption, BirdOption, varargin)
...
for i = 1:length(IndividualBirds),
    disp('Adjusting log amplitude values for all motif syllables ...');
    AdjustedLogAmp_FileName = [BirdNames{i}, '.AdjustedLogAmpValues.mat'];
    if (exist(AdjustedLogAmp_FileName, 'file'))
        load(AdjustedLogAmp_FileName);
        IndividualBirds(i).RawSyllLogAmplitudeMeanValue = RawSyllLogAmplitudeMeanValue;
        IndividualBirds(i).AdjustedSyllLogAmplitudeMeanValue = AdjustedSyllLogAmplitudeMeanValue;
    else
        ...
        RawSyllLogAmplitudeMeanValue = IndividualBirds(i).RawSyllLogAmplitudeMeanValue;
        AdjustedSyllLogAmplitudeMeanValue = IndividualBirds(i).AdjustedSyllLogAmplitudeMeanValue;
        save(AdjustedLogAmp_FileName, 'RawSyllLogAmplitudeMeanValue', 'AdjustedSyllLogAmplitudeMeanValue');
    end
end

% Now get the song categorisation and the female response for each of the
% syllables and the condition under which it was recorded (L0, L1, etc.).
for i = 1:length(IndividualBirds),
    for j = 1:size(IndividualBirds(i).AllSyllableData, 1),
        BoutIndexColumnIndex = find(cellfun(@length,strfind(IndividualBirds(i).BoutStatisticsColumnNames, 'BoutIndex')));
        BoutIndex = find(IndividualBirds(i).BoutStatistics(:,BoutIndexColumnIndex) == IndividualBirds(i).AllSyllableListBoutNum(j));
        if (~isempty(BoutIndex))
            IndividualBirds(i).AllSyllableCategorisation{j} = IndividualBirds(i).BoutCategorisation{BoutIndex};
            IndividualBirds(i).AllSyllableFemaleResponse{j} = IndividualBirds(i).BoutFemaleResponse{BoutIndex};
        else
            IndividualBirds(i).AllSyllableCategorisation{j} = 'NA';
            IndividualBirds(i).AllSyllableFemaleResponse{j} = 'NA';
        end
        if (strmatch(IndividualBirds(i).AllSyllableCategorisation{j}, 'D', 'exact'))
            IndividualBirds(i).AllSyllableCategorisationIndex(j) = 0;
        else
            if (strmatch(IndividualBirds(i).AllSyllableCategorisation{j}, 'DUN', 'exact'))
                IndividualBirds(i).AllSyllableCategorisationIndex(j) = 1;
            else
                if (strmatch(IndividualBirds(i).AllSyllableCategorisation{j}, 'UN', 'exact'))
                    IndividualBirds(i).AllSyllableCategorisationIndex(j) = 2;
                else
                    IndividualBirds(i).AllSyllableCategorisationIndex(j) = NaN;
                end
            end
        end
        IndividualBirds(i).AllSyllableTime(j) = IndividualBirds(i).FileTime(IndividualBirds(i).AllSyllableData(j,2)) + (IndividualBirds(i).AllSyllableData(j,4)/(1000 * 3600));
    end
end
