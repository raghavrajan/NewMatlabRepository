...
for i = 1:length(IndividualBirds),
    ...
    for j = 1:length(IndividualBirds(i).Conditions),
        ...
        if (length(DirSongIndices) >= 3)
            plot(min(IndividualBirds(i).BoutStatistics(:, ColumnIndex)):5:max(IndividualBirds(i).BoutStatistics(:, ColumnIndex)), histc(IndividualBirds(i).BoutStatistics(DirSongIndices, ColumnIndex), min(IndividualBirds(i).BoutStatistics(:, ColumnIndex)):5:max(IndividualBirds(i).BoutStatistics(:,ColumnIndex)))/length(DirSongIndices), [Colours(mod(j, length(Colours)) + 1), 'o-'], 'LineWidth', 2);
    ...
    axis tight;
    Temp = axis;
    Temp = [(min(IndividualBirds(i).BoutStatistics(:,ColumnIndex)) - 10) (max(IndividualBirds(i).BoutStatistics(:,ColumnIndex)) + 10) 0 1.35];
    axis(Temp);
    legend(PlotLegend{i});
