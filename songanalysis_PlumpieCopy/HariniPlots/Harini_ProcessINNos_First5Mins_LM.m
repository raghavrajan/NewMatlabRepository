function [DirBout_Stats, UndirBout_Stats, DirUndirBout_Stats] = Harini_ProcessINNos_LM(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)
...
UndirSongStartTime = 20; % in minutes
...
for i = 1:length(IndividualBirds),
    ...
    for j = min(IndividualBirds(i).BoutStatistics(:,ConditionColumnIndex)):max(IndividualBirds(i).BoutStatistics(:,ConditionColumnIndex)),
        ...
        if (j ~= 6)
            UndirBouts{i}{j} = AllBouts(find(AllBoutTimes >= UndirSongStartTime));
        end
...
print(fullfile(FinalFigureDir, ['First5Mins_AllBirdsExceptExamples.eps']), '-depsc2', '-r600');
print(fullfile(FinalFigureDir, ['First5Mins_AllBirdsExceptExamples.png']), '-dpng', '-r600');
...
set(gcf, 'PaperPositionMode', 'auto');
print(fullfile(FinalFigureDir, ['First5Mins_Fig.3.ResponseExamples_Counts.eps']), '-depsc2', '-r600');
print(fullfile(FinalFigureDir, ['First5Mins_Fig.3.ResponseExamples_Counts.png']), '-dpng', '-r600');
...
print(fullfile(FinalFigureDir, ['First5Mins_ResponseSlopes.eps']), '-depsc2', '-r600');
print(fullfile(FinalFigureDir, ['First5Mins_ResponseSlopes.png']), '-dpng', '-r600');
