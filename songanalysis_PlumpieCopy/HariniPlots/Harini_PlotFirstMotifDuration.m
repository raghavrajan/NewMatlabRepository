function [] = Harini_PlotFirstMotifDuration(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)
...
%   b) Correlation to see if distance and First motif duration (msec) is related - spearman's
%   rank correlation 
% =========================================================================
...
Fid = fopen('FirstMotifDuration_Correlations.txt', 'w');
...
for i = 1:length(IndividualBirds),
    ...
    for j = UniqueConditions(:)',
        ...
        plot(min(IndividualBirds(i).BoutStatistics(:, ColumnIndex)):5:max(IndividualBirds(i).BoutStatistics(:, ColumnIndex)), histc(IndividualBird_AllBouts{i}(ConditionIndices, 1), min(IndividualBirds(i).BoutStatistics(:, ColumnIndex)):5:max(IndividualBirds(i).BoutStatistics(:,ColumnIndex)))/length(ConditionIndices), [Colours(mod(j, length(Colours)) + 1), 'o-'], 'LineWidth', 2);
    ...
    Temp = [min(IndividualBirds(i).BoutStatistics(:, ColumnIndex)) (max(IndividualBirds(i).BoutStatistics(:,ColumnIndex)) + 0.5) 0 1.35];
    axis(Temp);
    legend(PlotLegend{i}, 'Location', 'northwest');
    xlabel('First motif duration (msec)');
    ...
    for j = UniqueConditions(:)',
        ...
        plot(min(IndividualBirds(i).BoutStatistics(:, ColumnIndex)):5:max(IndividualBirds(i).BoutStatistics(:, ColumnIndex)), histc(IndividualBird_DirectedBouts{i}(ConditionIndices, 1), min(IndividualBirds(i).BoutStatistics(:, ColumnIndex)):5:max(IndividualBirds(i).BoutStatistics(:,ColumnIndex)))/length(ConditionIndices), [Colours(mod(j, length(Colours)) + 1), 'o-'], 'LineWidth', 2);
        PlotLegend{i}{end+1} = IndividualBirds(i).Conditions{j};
    end
    ...
    for j = UniqueConditions(:)',
        ...
        plot(min(IndividualBirds(i).BoutStatistics(:, ColumnIndex)):5:max(IndividualBirds(i).BoutStatistics(:, ColumnIndex)), histc(IndividualBird_UnDirectedBouts{i}(ConditionIndices, 1), min(IndividualBirds(i).BoutStatistics(:, ColumnIndex)):5:max(IndividualBirds(i).BoutStatistics(:,ColumnIndex)))/length(ConditionIndices), [Colours(mod(j, length(Colours)) + 1), 'o-'], 'LineWidth', 2);
        PlotLegend{i}{end+1} = IndividualBirds(i).Conditions{j};
    end
    ...
    axis tight;
    Temp = axis;
    Temp = [(ConditionDistances(1) - 1) (ConditionDistances(end) + 1) 0.85*Temp(3) 1.05*Temp(4)];
    axis(Temp);
    legend('All bouts', 'Directed bouts', 'Undirected bouts', 'Location', 'southeast');
    ylabel('Mean First motif duration (msec)');
...
ylabel('% change in mean First motif duration (msec) relative to Undirected song');
