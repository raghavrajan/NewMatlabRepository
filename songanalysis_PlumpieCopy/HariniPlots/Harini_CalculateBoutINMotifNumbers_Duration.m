function [Bout_Stats] = Harini_CalculateBoutINMotifNumbers_Duration(IndividualBirds, Bouts, MotifLabels, INLabels, CommonMotifs)
...
for k = Bouts(:)',
    ...
    BoutSAPFF = IndividualBirds.AllSyllableFFAutocorrKao(IndividualBirds.Bouts(k,1):IndividualBirds.Bouts(k,2));
    BoutSAPFF = BoutSAPFF(:);
    
    LogAmpColIndex = find(cellfun(@length, strfind(IndividualBirds.SortedBirdParameters(1).SAPFeat_FieldNames, 'LogAmplitude')));
    BoutSAPLogAmp = IndividualBirds.AllSyllableFeatValues(IndividualBirds.Bouts(k,1):IndividualBirds.Bouts(k,2),LogAmpColIndex);
