function [OutputStats] = Harini_DoStats_AndPlots_FF(DirBout_Stats, DirUndirBout_Stats, UndirBout_Stats, StructString, MinTrialNo, Distances, LabelString, BirdNames, ParametricOrNot, SyllIndex)
...
for i = 1:length(DirBout_Stats)
        ...
        if (~isempty(DirBout_Stats{i}{j}))
            for SyllIndex = 1:length(eval(['DirBout_Stats{i}{j}.', StructString])),
                if (eval(['length(~isnan(DirBout_Stats{i}{j}.', StructString, '{', num2str(SyllIndex), '}(:)))']) >= MinTrialNo)
                    ...
                    GLMGroupData{i}{SyllIndex} = [GroupData{i}{SyllIndex}; [eval(['DirBout_Stats{i}{j}.', StructString, '{', num2str(SyllIndex), '}(:)']) ones(size(eval(['DirBout_Stats{i}{j}.', StructString, '{', num2str(SyllIndex), '}(:)'])))*j ones(size(eval(['DirBout_Stats{i}{j}.', StructString, '{', num2str(SyllIndex), '}(:)'])))*1]];
                else
                    GLMGroupData{i}{SyllIndex} = [GLMGroupData{i}{SyllIndex}; [ones(MinTrialNo,1)*NaN ones(MinTrialNo,1)*j ones(MinTrialNo,1)*1]];
                end
            end
        else
            GLMGroupData{i}{SyllIndex} = [GLMGroupData{i}{SyllIndex}; [ones(MinTrialNo,1)*NaN ones(MinTrialNo,1)*j ones(MinTrialNo,1)*1]];
        ...
        if (~isempty(DirUndirBout_Stats{i}{j}))
            for SyllIndex = 1:length(eval(['DirUndirBout_Stats{i}{j}.', StructString])),
                if (eval(['length(~isnan(DirUndirBout_Stats{i}{j}.', StructString, '{', num2str(SyllIndex), '}(:)))']) >= MinTrialNo)
                    ...
                    GLMGroupData{i}{SyllIndex} = [GroupData{i}{SyllIndex}; [eval(['DirUndirBout_Stats{i}{j}.', StructString, '{', num2str(SyllIndex), '}(:)']) ones(size(eval(['DirUndirBout_Stats{i}{j}.', StructString, '{', num2str(SyllIndex), '}(:)'])))*j ones(size(eval(['DirUndirBout_Stats{i}{j}.', StructString, '{', num2str(SyllIndex), '}(:)'])))*2]];
                else
                    GLMGroupData{i}{SyllIndex} = [GLMGroupData{i}{SyllIndex}; [ones(MinTrialNo,1)*NaN ones(MinTrialNo,1)*j ones(MinTrialNo,1)*2]];
                end
            end
        else
            GLMGroupData{i}{SyllIndex} = [GLMGroupData{i}{SyllIndex}; [ones(MinTrialNo,1)*NaN ones(MinTrialNo,1)*j ones(MinTrialNo,1)*2]];
        ...
        if (~isempty(UndirBout_Stats{i}{j}))
            for SyllIndex = 1:length(eval(['UndirBout_Stats{i}{j}.', StructString])),
                if (eval(['length(~isnan(UndirBout_Stats{i}{j}.', StructString, '{', num2str(SyllIndex), '}(:)))']) >= MinTrialNo)
                    ...
                    GLMGroupData{i}{SyllIndex} = [GroupData{i}{SyllIndex}; [eval(['UndirBout_Stats{i}{j}.', StructString, '{', num2str(SyllIndex), '}(:)']) ones(size(eval(['UndirBout_Stats{i}{j}.', StructString, '{', num2str(SyllIndex), '}(:)'])))*j ones(size(eval(['UndirBout_Stats{i}{j}.', StructString, '{', num2str(SyllIndex), '}(:)'])))*3]];
                else
                    GLMGroupData{i}{SyllIndex} = [GLMGroupData{i}{SyllIndex}; [ones(MinTrialNo,1)*NaN ones(MinTrialNo,1)*j ones(MinTrialNo,1)*3]];
                end
            end
        else
            GLMGroupData{i}{SyllIndex} = [GLMGroupData{i}{SyllIndex}; [ones(MinTrialNo,1)*NaN ones(MinTrialNo,1)*j ones(MinTrialNo,1)*3]];
    ...
    for SyllIndex = 1:length(GroupData{i}),
        ...
        TempDataIndices = find(GLMGroupData{i}{SyllIndex}(:,3) == 2);
        Temp2DataIndices = find(GLMGroupData{i}{SyllIndex}(:,2) == 6);
        GLMModelDataIndices = setdiff(1:1:size(GroupData{i}{SyllIndex},1), union(TempDataIndices, Temp2DataIndices));
        
        DistrName = 'normal';
        ContextValues = {'D' 'DUN' 'UN'};
        ContextNumericalValues = [0 2 1];

        LogAmp = GLMGroupData{i}{SyllIndex}(GLMModelDataIndices,1);
        Contexts = ContextValues(GLMGroupData{i}{SyllIndex}(GLMModelDataIndices,3));
        DistanceValues = Distances(GLMGroupData{i}{SyllIndex}(GLMModelDataIndices,2));

        Context_NumericalValues = ContextNumericalValues(GLMGroupData{i}{SyllIndex}(GLMModelDataIndices, 3));

        LogAmp = LogAmp(:);
        Contexts = Contexts(:);
        DistanceValues = DistanceValues(:);

        GLM_DataTable = table(DistanceValues, Contexts, LogAmp);
        mdl = fitglm(GLM_DataTable, 'interactions', 'CategoricalVars', [2], 'Distribution', DistrName);

        mdl3 = fitglm(zscore([DistanceValues(:) Context_NumericalValues(:)]), (LogAmp(:) - nanmean(LogAmp))/nanstd(LogAmp), 'interactions', 'CategoricalVars', [2], 'Distribution', DistrName);
        Temp_Estimates_PValues = mdl3.Coefficients{:, {'Estimate', 'pValue'}};

        if (size(Temp_Estimates_PValues,1) == 4)
            GLMFit_Intercept{i}(SyllIndex,:) = Temp_Estimates_PValues(1,:);
            GLMFit_Distance{i}(SyllIndex,:) = Temp_Estimates_PValues(2,:);
            GLMFit_Context{i}(SyllIndex,:) = Temp_Estimates_PValues(3,:);
            GLMFit_DistanceContext_Interactions{i}(SyllIndex,:) = Temp_Estimates_PValues(4,:);
        else
            if (size(Temp_Estimates_PValues,1) == 3)
                GLMFit_Intercept{i}(SyllIndex,:) = Temp_Estimates_PValues(1,:);
                GLMFit_Distance{i}(SyllIndex,:) = Temp_Estimates_PValues(2,:);
                GLMFit_Context{i}(SyllIndex,:) = Temp_Estimates_PValues(3,:);
                GLMFit_DistanceContext_Interactions{i}(SyllIndex,:) = ones(1,2)*NaN;
            else
                GLMFit_Intercept{i}(SyllIndex,:) = ones(1,2)*NaN;
                GLMFit_Distance{i}(SyllIndex,:) = ones(1,2)*NaN;
                GLMFit_Context{i}(SyllIndex,:) = ones(1,2)*NaN;
                GLMFit_DistanceContext_Interactions{i}(SyllIndex,:) = ones(1,2)*NaN;
            end
        end
        disp(mdl);
