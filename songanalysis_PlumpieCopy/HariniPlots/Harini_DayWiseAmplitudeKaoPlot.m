function [] = Harini_DayWiseAmplitudeKaoPlot(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)
    ...
    for i = 1:length(BirdNames),
        ...
        for j = 1:length(TempBirdParameters(1).MotifLabels),
            ...
            for k = 1:length(TempBirdParameters),
                SyllableIndices = find((char(TempBirdParameters(k).SyllableData(:,1)) == TempBirdParameters(1).MotifLabels(j)) & (~isnan(TempBirdParameters(k).AmplitudeKao(:)))); % The second condition here ensures that the log amplitude is not NaN
                AllFeatValues = [AllFeatValues; [ones(length(SyllableIndices),1)*k TempBirdParameters(k).AmplitudeKao(SyllableIndices)]];
                plot(ones(length(SyllableIndices))*(k-0.2), TempBirdParameters(k).AmplitudeKao(SyllableIndices), 'k.', 'MarkerSize', 6);
                errorbar(k, eval([PlotType, '(TempBirdParameters(k).AmplitudeKao(SyllableIndices))']), std(TempBirdParameters(k).AmplitudeKao(SyllableIndices)), 'rs', 'LineWidth', 2, 'MarkerSize', 6);
                SessionMeans{j}(k,:) = [eval([PlotType, '(TempBirdParameters(k).AmplitudeKao(SyllableIndices))']), std(TempBirdParameters(k).AmplitudeKao(SyllableIndices))];
                SessionCVs{j}(k,:) = [nanstd(TempBirdParameters(k).AmplitudeKao(SyllableIndices))/nanmean(TempBirdParameters(k).AmplitudeKao(SyllableIndices))];
