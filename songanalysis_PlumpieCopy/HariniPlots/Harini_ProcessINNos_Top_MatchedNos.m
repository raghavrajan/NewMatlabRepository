function [DirBout_Stats, UndirBout_Stats, DirUndirBout_Stats] = Harini_ProcessINNos_Top_MatchedNos(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)
...
for i = 1:length(IndividualBirds),
    ...
    for j = min(IndividualBirds(i).BoutStatistics(:,ConditionColumnIndex)):max(IndividualBirds(i).BoutStatistics(:,ConditionColumnIndex)),
        ...
        if (j ~= 6)
            [SortedData, SortedDataIndices] = sort(AllBoutStats{i}{j}.TotalINNumber_500ms, 'descend');
            AllBoutStats{i}{j}.TotalINNumber_500ms = AllBoutStats{i}{j}.TotalINNumber_500ms(SortedDataIndices);
            
            [SortedData, SortedDataIndices] = sort(AllBoutStats{i}{j}.CompleteMotifNumber, 'descend');
            AllBoutStats{i}{j}.CompleteMotifNumber = AllBoutStats{i}{j}.CompleteMotifNumber(SortedDataIndices);
            
            [SortedData, SortedDataIndices] = sort(AllBoutStats{i}{j}.FirstMotifDuration, 'ascend');
            AllBoutStats{i}{j}.FirstMotifDuration = AllBoutStats{i}{j}.FirstMotifDuration(SortedDataIndices);

            if (~isempty(SortedDataIndices))
                DirBout_Stats{i}{j}.TotalINNumber_500ms = AllBoutStats{i}{j}.TotalINNumber_500ms(1:length(DirBouts{i}{j}));
                UnDirBout_Stats{i}{j}.TotalINNumber_500ms = AllBoutStats{i}{j}.TotalINNumber_500ms((length(SortedDataIndices) - length(UndirBouts{i}{j}))+1:end);
            
                DirBout_Stats{i}{j}.CompleteMotifNumber = AllBoutStats{i}{j}.CompleteMotifNumber(1:length(DirBouts{i}{j}));
                UnDirBout_Stats{i}{j}.CompleteMotifNumber = AllBoutStats{i}{j}.CompleteMotifNumber((length(SortedDataIndices) - length(UndirBouts{i}{j}))+1:end);
                
                DirBout_Stats{i}{j}.FirstMotifDuration = AllBoutStats{i}{j}.FirstMotifDuration(1:length(DirBouts{i}{j}));
                UnDirBout_Stats{i}{j}.FirstMotifDuration = AllBoutStats{i}{j}.FirstMotifDuration((length(SortedDataIndices) - length(UndirBouts{i}{j}))+1:end);
            else
                DirBout_Stats{i}{j} = [];
                UnDirBout_Stats{i}{j} = [];
            end
...
LabelString = 'TopMatchedNos';
MinTrialNo = 3;
StructString = 'Test';

Harini_AllFeatureStats_Plots_TopMatched(DirBout_Stats, DirUndirBout_Stats, UndirBout_Stats, StructString, MinTrialNo, Distances, LabelString, BirdNames);
...
FinalFigureDir = pwd;
print(fullfile(FinalFigureDir, ['AllBirds.TopMatchedNos.BoutLevelFeatures.eps']), '-depsc2', '-r600');
print(fullfile(FinalFigureDir, ['AllBirds.TopMatchedNos.BoutLevelFeatures.png']), '-dpng', '-r600');
