function [L0Dir_Undir_PValue] = Harini_AllFeatureStats_Plots_OnlyDir(DirBout_Stats, DirUndirBout_Stats, UndirBout_Stats, StructString, MinTrialNo, Distances, LabelString, BirdNames)
...
FeaturesToConsider = {'INNumber_500ms_Time' 'BoutLength' 'FirstMotifDuration'};
...
for i = 1:length(UnNormalizedDistanceDirMeans),
    FeatureDiffs{i} = (DistanceDirMeans{i} - repmat(DistanceUnDirMeans{i}(:,end), 1, size(DistanceDirMeans{i},2))).^2;
end
...
%    errorbar(Distances(end)-10, nanmean(DistanceUnDirMeans{i}(:,end)), nanstd(DistanceUnDirMeans{i}(:,end))/sqrt(length(find(~isnan(DistanceUnDirMeans{i}(:,end))))), 'ks-', 'LineWidth', 1.5, 'MarkerSize', 8);
    ...
    % Now to do the correlation and put the r and p values in the corner
    [CorrR, CorrP] = corr(Distances(reshape(repmat(1:1:5, 14, 1), 14*5, 1))', reshape(FeatureDiffs{i}(:,1:5), 14*5, 1), 'rows', 'complete', 'type', 'Pearson');
    text(130, Temp(4)/1.08, {['r = ', num2str(CorrR)]; ['p = ', num2str(CorrP)]});
end
...
% Now to do the correlation and put the r and p values in the corner
[CorrR, CorrP] = corr(Distances(reshape(repmat(1:1:5, 14, 1), 14*5, 1))', reshape(FFCVDiffs(:,1:5), 14*5, 1), 'rows', 'complete', 'type', 'Pearson');
text(130, Temp(4)/1.08, {['r = ', num2str(CorrR)]; ['p = ', num2str(CorrP)]});
