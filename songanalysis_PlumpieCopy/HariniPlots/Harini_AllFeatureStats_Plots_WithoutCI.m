function [L0Dir_Undir_PValue] = Harini_AllFeatureStats_Plots_WithoutCI(DirBout_Stats, DirUndirBout_Stats, UndirBout_Stats, StructString, MinTrialNo, Distances, LabelString, BirdNames)
...
% p(4,2).select();
% hold on;
% plot(Distances(1:5), AllBirdDistanceFFUnDirCVs(:,1:5)', 'ko-', 'LineWidth', 0.25, 'MarkerSize', 4);
% for j = 1:5,
%     if (~isempty(find(~isnan(AllBirdDistanceFFUnDirCVs(:,j)))))
%         MeanSEM(j,:) = [nanmean(AllBirdDistanceFFUnDirCVs(:,j)) nanstd(AllBirdDistanceFFUnDirCVs(:,j))/sqrt(length(find(~isnan(AllBirdDistanceFFUnDirCVs(:,j)))))];
%     else
%         MeanSEM(j,:) = [NaN NaN];
%     end
% end
...
% axis tight;
% Temp2 = axis;
% Temp = [-10 Distances(end)+5 0 1.25*max(Temp1(4), Temp2(4))];
% axis(Temp);
% plot(Temp(1:2), [0 0], 'k--');
% set(gca, 'XTick', [0 20 60 110 165 195], 'XTickLabel', {'0' '20' '60' '110' '165' 'UN'});
% clear MeanSEM;
...
% 
% % Now to do the correlation and put the r and p values in the corner
% [CorrR, CorrP] = corr(Distances(reshape(repmat(1:1:5, 14, 1), 14*5, 1))', reshape(AllBirdDistanceFFDirCVs(:,1:5), 14*5, 1), 'rows', 'complete', 'type', 'Pearson');
% text(130, Temp(4)/1.08, {['r = ', num2str(CorrR)]; ['p = ', num2str(CorrP)]});
% 
% p(4,2).select();
% % Now to do the correlation and put the r and p values in the corner
% [CorrR, CorrP] = corr(Distances(reshape(repmat(1:1:5, 14, 1), 14*5, 1))', reshape(AllBirdDistanceFFUnDirCVs(:,1:5), 14*5, 1), 'rows', 'complete', 'type', 'Pearson');
% text(130, Temp(4)/1.08, {['r = ', num2str(CorrR)]; ['p = ', num2str(CorrP)]});
