function [] = Harini_DayWiseAmplitudeRMSPlot(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)
    ...
    for i = 1:length(BirdNames),
        ...
        for j = 1:length(TempBirdParameters(1).MotifLabels),
            ...
            for k = 1:length(TempBirdParameters),
                SyllableIndices = find((char(TempBirdParameters(k).SyllableData(:,1)) == TempBirdParameters(1).MotifLabels(j)) & (~isnan(TempBirdParameters(k).AmplitudeRMS(:)))); % The second condition here ensures that the log amplitude is not NaN
                AllFeatValues = [AllFeatValues; [ones(length(SyllableIndices),1)*k TempBirdParameters(k).AmplitudeRMS(SyllableIndices)]];
                plot(ones(length(SyllableIndices))*(k-0.2), TempBirdParameters(k).AmplitudeRMS(SyllableIndices), 'k.', 'MarkerSize', 6);
                errorbar(k, eval([PlotType, '(TempBirdParameters(k).AmplitudeRMS(SyllableIndices))']), std(TempBirdParameters(k).AmplitudeRMS(SyllableIndices)), 'rs', 'LineWidth', 2, 'MarkerSize', 6);
                SessionMeans{j}(k,:) = [eval([PlotType, '(TempBirdParameters(k).AmplitudeRMS(SyllableIndices))']), std(TempBirdParameters(k).AmplitudeRMS(SyllableIndices))];
                SessionCVs{j}(k,:) = [nanstd(TempBirdParameters(k).AmplitudeRMS(SyllableIndices))/nanmean(TempBirdParameters(k).AmplitudeRMS(SyllableIndices))];
