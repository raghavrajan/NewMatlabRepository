function [] = Harini_PlotAmplitude_OnlyHF(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)
...
for i = 1:length(IndividualBirds),
    ...
    if (sum(strcmp(IndividualBirds(i).Microphone, 'HF')) < 1)
        continue;
    end
    ...
    for FFSylls = 1:length(IndividualBird_AllBouts{i}),
        ...
        if (isempty(IndividualBird_AllBouts{i}{FFSylls}))
            continue;
        end
...
DirectedBouts = [];
for i = 1:length(DirectedBouts_L1),
    if (isempty(DirectedBouts_L0{i}))
        if (isempty(DirectedBouts_UN{i}))
            DirectedBouts = [DirectedBouts; [ones(size(DirectedBouts_L1{i}(:)))*NaN DirectedBouts_L1{i}(:) DirectedBouts_L2{i}(:) DirectedBouts_L3{i}(:) DirectedBouts_L4{i}(:) ones(size(DirectedBouts_L1{i}(:)))*NaN]];
        else
...
UnDirectedBouts = [];
for i = 1:length(UnDirectedBouts_L1),
    if (isempty(UnDirectedBouts_L0{i}))
        if (isempty(UnDirectedBouts_UN{i}))
            UnDirectedBouts = [UnDirectedBouts; [ones(size(UnDirectedBouts_L1{i}(:)))*NaN UnDirectedBouts_L1{i}(:) UnDirectedBouts_L2{i}(:) UnDirectedBouts_L3{i}(:) UnDirectedBouts_L4{i}(:) ones(size(UnDirectedBouts_L1{i}(:)))*NaN]];
        else
...
for i = 1:length(IndividualBirds),
    ...
    for FFSyll = 1:length(IndividualBird_AllBouts{i}),
        ...
        if (isempty(IndividualBird_AllBouts{i}{FFSyll}))
            continue;
        end
...
set(gca, 'XTick', ConditionDistances, 'XTickLabel', AxisLabelString);
xlabel('Distance of female cage (cm)');
ylabel('\Delta change in mean amplitude relative to Undirected song');
title(['All bouts: (n=', num2str(length(BirdNames)), ') birds']);
axis tight;
Temp = axis;
Temp = [(ConditionDistances(1) - 12) (ConditionDistances(end) + 12) 1.02*Temp(3) 1.2*Temp(4)];
...
% Now plotting only L0 and undirected for all bouts, directed bouts and
% undirected bouts
figure; % Means
hold on;
% for i = 1:length(IndividualBird_AllBouts),
%     for j = 1:length(IndividualBird_AllBouts{i}),
%         L0Trials = find((IndividualBird_AllBouts{i}{j}(:,2) == 1) & ~isnan(IndividualBird_AllBouts{i}{j}(:,1)));
%         UNTrials = find((IndividualBird_AllBouts{i}{j}(:,2) == 6) & ~isnan(IndividualBird_AllBouts{i}{j}(:,1)));
%         
%         plot(nanmean(IndividualBird_AllBouts{i}{j}(L0Trials,1)), nanmean(IndividualBird_AllBouts{i}{j}(UNTrials, 1)), 'ko');
%         % Now plot the error bars
%         plot(ones(1,2) * nanmean(IndividualBird_AllBouts{i}{j}(L0Trials,1)), [(nanmean(IndividualBird_AllBouts{i}{j}(UNTrials, 1)) - nanstd(IndividualBird_AllBouts{i}{j}(UNTrials, 1))/sqrt(length(UNTrials))) (nanmean(IndividualBird_AllBouts{i}{j}(UNTrials, 1)) + nanstd(IndividualBird_AllBouts{i}{j}(UNTrials, 1))/sqrt(length(UNTrials)))], 'k');
%         plot([(nanmean(IndividualBird_AllBouts{i}{j}(L0Trials, 1)) - nanstd(IndividualBird_AllBouts{i}{j}(L0Trials, 1))/sqrt(length(L0Trials))) (nanmean(IndividualBird_AllBouts{i}{j}(L0Trials, 1)) + nanstd(IndividualBird_AllBouts{i}{j}(L0Trials, 1))/sqrt(length(L0Trials)))], ones(1,2) * nanmean(IndividualBird_AllBouts{i}{j}(UNTrials,1)), 'k');
%     end
% end
figure;
hold on;
...
for i = 1:length(IndividualBird_DirectedBouts),
    for j = 1:length(IndividualBird_DirectedBouts{i}),
        L0Trials = find((IndividualBird_DirectedBouts{i}{j}(:,2) == 1) & ~isnan(IndividualBird_DirectedBouts{i}{j}(:,1)));
        UNTrials = find((IndividualBird_DirectedBouts{i}{j}(:,2) == 6) & ~isnan(IndividualBird_DirectedBouts{i}{j}(:,1)));
        [H, P] = ttest2(IndividualBird_DirectedBouts{i}{j}(L0Trials, 1), IndividualBird_DirectedBouts{i}{j}(UNTrials, 1));
        if (P < 0.05)
            plot([1.1 1.9], [mean(IndividualBird_DirectedBouts{i}{j}(L0Trials, 1)) mean(IndividualBird_DirectedBouts{i}{j}(UNTrials, 1))], 'ko-', 'MarkerFaceColor', 'k');
        else
            plot([1.1 1.9], [mean(IndividualBird_DirectedBouts{i}{j}(L0Trials, 1)) mean(IndividualBird_DirectedBouts{i}{j}(UNTrials, 1))], 'ko-');
        end
        Means(end+1,:) = [mean(IndividualBird_DirectedBouts{i}{j}(L0Trials, 1)) mean(IndividualBird_DirectedBouts{i}{j}(UNTrials, 1))];
    end
end
L0UNBar = bar(mean(Means));
set(L0UNBar, 'FaceColor', 'none', 'EdgeColor', 'k');
errorbar([1 2], mean(Means), std(Means)/sqrt(length(size(Means, 1))), 'ko');
set(gca, 'XTick', [1 2], 'XTickLabel', [{'L0 - Directed songs'} {'Undirected songs'}]);
ylabel('Mean Log Amplitude (dB)');
title('Comparison of L0 directed and UN songs'); 
...
%         [H, P] = ttest2(IndividualBird_DirectedBouts{i}{j}(L0Trials, 1), IndividualBird_DirectedBouts{i}{j}(UNTrials, 1));
%         if (P < 0.05)
%             plot([1.1 1.9], [median(IndividualBird_DirectedBouts{i}{j}(L0Trials, 1)) median(IndividualBird_DirectedBouts{i}{j}(UNTrials, 1))], 'ko-', 'MarkerFaceColor', 'k');
%         else
%             plot([1.1 1.9], [median(IndividualBird_DirectedBouts{i}{j}(L0Trials, 1)) median(IndividualBird_DirectedBouts{i}{j}(UNTrials, 1))], 'ko-');
%         end
%         Medians(end+1,:) = [mean(IndividualBird_DirectedBouts{i}{j}(L0Trials, 1)) median(IndividualBird_DirectedBouts{i}{j}(UNTrials, 1))];
%     end
% end
% L0UNBar = bar(mean(Medians));
% set(L0UNBar, 'FaceColor', 'none', 'EdgeColor', 'k');
% errorbar([1 2], mean(Medians), std(Medians)/sqrt(length(size(Medians, 1))), 'ko');
% set(gca, 'XTick', [1 2], 'XTickLabel', [{'L0 - Directed songs'} {'Undirected songs'}]);
% ylabel('Median Log Amplitude (dB)');
% title('Comparison of L0 directed and UN songs');

% for i = 1:length(IndividualBird_UnDirectedBouts),
%     for j = 1:length(IndividualBird_UnDirectedBouts{i}),
%         L0Trials = find((IndividualBird_UnDirectedBouts{i}{j}(:,2) == 1) & ~isnan(IndividualBird_UnDirectedBouts{i}{j}(:,1)));
%         UNTrials = find((IndividualBird_UnDirectedBouts{i}{j}(:,2) == 6) & ~isnan(IndividualBird_UnDirectedBouts{i}{j}(:,1)));
%         
%         plot(nanmean(IndividualBird_UnDirectedBouts{i}{j}(L0Trials,1)), nanmean(IndividualBird_UnDirectedBouts{i}{j}(UNTrials, 1)), 'bo');
%         % Now plot the error bars
%         plot(ones(1,2) * nanmean(IndividualBird_UnDirectedBouts{i}{j}(L0Trials,1)), [(nanmean(IndividualBird_UnDirectedBouts{i}{j}(UNTrials, 1)) - nanstd(IndividualBird_UnDirectedBouts{i}{j}(UNTrials, 1))/sqrt(length(UNTrials))) (nanmean(IndividualBird_UnDirectedBouts{i}{j}(UNTrials, 1)) + nanstd(IndividualBird_UnDirectedBouts{i}{j}(UNTrials, 1))/sqrt(length(UNTrials)))], 'b');
%         plot([(nanmean(IndividualBird_UnDirectedBouts{i}{j}(L0Trials, 1)) - nanstd(IndividualBird_UnDirectedBouts{i}{j}(L0Trials, 1))/sqrt(length(L0Trials))) (nanmean(IndividualBird_UnDirectedBouts{i}{j}(L0Trials, 1)) + nanstd(IndividualBird_UnDirectedBouts{i}{j}(L0Trials, 1))/sqrt(length(L0Trials)))], ones(1,2) * nanmean(IndividualBird_UnDirectedBouts{i}{j}(UNTrials,1)), 'b');
%     end
% end
