function [L0Dir_Undir_PValue] = Harini_FeatureFits_INNum_MotifNumDur(DirBout_Stats, DirUndirBout_Stats, UndirBout_Stats, Distances, BirdNames)
...
for i = 1:length(DirBout_Stats),
    ...
    print(fullfile(FinalFigureDir, [BirdNames{i}, 'FeatureFits_INNum_MotifNum_MotifDur.eps']), '-depsc2', '-r600');
    print(fullfile(FinalFigureDir, [BirdNames{i}, 'FeatureFits_INNum_MotifNum_MotifDur.png']), '-dpng', '-r600');
end
...
for BirdIndex = 1:length(BirdsToPlot),
    ...
    for Feat = 1:length(FeaturesToConsider),
        ...
        % Now best fit line
        RobustCoeffs = FitCoeffs{Feat}(i,:);
        FixedX_BootStrapCoeffs = BootStrapCI{Feat}{i};
        ...
        axis(Temp);
        plot([-5 Temp(2)+5], ones(1,2)*RobustCoeffs(1), 'k--');
        text(Temp(2)-23, Temp(3), '//', 'FontSize', 12)
        %text(Temp(1)+2, Temp(4)/1.15, {['Rsq = ', num2str(Rsq)]; ['p = ', num2str(RandomProb(end))]});
        %text(Temp(1)+2, Temp(4)/1.05, {['Slope = ', num2str(FixedX_BootStrapCoeffs(1,end)), ' : ', num2str(FixedX_BootStrapCoeffs(2,end)), ' : ', num2str(FixedX_BootStrapCoeffs(3,end))]});
        ylabel(YLabelString{Feat})
%         if (BirdIndex == length(BirdsToPlot))
%             xlabel('Distance (cm)');
%         end
        DistanceXLabelString = [];
        DistanceXValues = [];
        for Dist = 1:length(unique(Distances(GroupData{i}(DirIndices,end-1)))),
            DistanceXValues(Dist) = Distances(Dist);
            DistanceXLabelString{Dist} = num2str(Distances(Dist));
        end
        DistanceXValues(end+1) = DistanceXValues(end) + 30;
    ...
    % Now for motif duration
    DirIndices = find(AllMotifDur_GroupData{i}(:,end) == 1);
    UnDirIndices = find((AllMotifDur_GroupData{i}(:,end-1) == 6) & (AllMotifDur_GroupData{i}(:,end) == 3));
    % First plot the group data
    p(BirdIndex,3).select();
    hold on;
    ...
    RobustCoeffs = FitCoeffs{3}(i,:);
    FixedX_BootStrapCoeffs = BootStrapCI{3}{i};
...
%     end
    DistanceXLabelString = [];
    DistanceXValues = [];
    for Dist = 1:length(unique(Distances(AllMotifDur_GroupData{i}(DirIndices,end-1)))),
        DistanceXValues(Dist) = Distances(Dist);
        DistanceXLabelString{Dist} = num2str(Distances(Dist));
    end
    DistanceXValues(end+1) = DistanceXValues(end) + 30;
...
set(gcf, 'Color', 'w');
set(gcf, 'Units', 'inches');
set(gcf, 'Position', [10 1.4 8.3 9.2]);
set(gcf, 'PaperPositionMode', 'auto');
print(fullfile(FinalFigureDir, ['AllBirds_Example_Summary_INNum_MotifNum_MotifDur.eps']), '-depsc2', '-r600');
print(fullfile(FinalFigureDir, ['AllBirds_Example_Summary_INNum_MotifNum_MotifDur.png']), '-dpng', '-r600');
...
set(gcf, 'PaperPositionMode', 'auto');
print(fullfile(FinalFigureDir, ['AllBirds_Slope_Summary_INNum_MotifNum_MotifDur.eps']), '-depsc2', '-r600');
print(fullfile(FinalFigureDir, ['AllBirds_Slope_Summary_INNum_MotifNum_MotifDur.png']), '-dpng', '-r600');

for i = 1:size(SlopeSignificance, 1),
    NumSig = size(SlopeSignificance, 2) - length(find(SlopeSignificance(i,:) == 0));
    NumIncrease = length(find(SlopeSignificance(i,:) == 1));
    NumDecrease = length(find(SlopeSignificance(i,:) == -1));
    disp([YLabelString{i}, ': # significant changes = ', num2str(NumSig), '/', num2str(size(SlopeSignificance,2)), 'birds; # significant increases = ', num2str(length(NumIncrease)), ' and # significant decrease = ', num2str(length(NumDecrease))]);
end
