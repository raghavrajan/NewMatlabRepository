function [BaselineSubtractedValues, RawValues] = Harini_CalcMeanMaxFFTLogAmplitude(IndividualBirds, BirdNames)
...
for i = 1:length(IndividualBirds),
    ...
    BaselineSubtractedValues(i).MaxFFTLogAmpValues = ones(size(IndividualBirds(i).AllFFTLogAmplitudes))*NaN;
    
    RawValues(i).MeanFFTLogAmpValues = ones(size(IndividualBirds(i).AllFFTLogAmplitudes))*NaN;
    RawValues(i).MaxFFTLogAmpValues = ones(size(IndividualBirds(i).AllFFTLogAmplitudes))*NaN;
...
%     end
    for j = 1:length(MotifSylls),
        ...
        for k = 1:length(MatchSyllables)
            if (IndividualBirds(i).AllSyllableLogAmpStatus(MatchSyllables(k)) == 0)
                continue;
            end
            ...
            Amp_XX = linspace(Amp_X(1), Amp_X(end), 10*length(Amp_X));
            UpSampledAmplitude = spline(Amp_X, IndividualBirds(i).AllFFTLogAmplitudes{MatchSyllables(k)}, Amp_XX);
            
            SyllStartIndex = find(Amp_XX >= Padding, 1, 'first');
            SyllEndIndex = find(Amp_XX <= (Amp_XX(end) - Padding), 1, 'last');
            AboveThreshold = ((UpSampledAmplitude >= ThresholdVal));
            ...
            if (SyllStartIndex > SyllEndIndex)
                continue;
            end
            
            RawValues(i).MeanFFTLogAmpValues(MatchSyllables(k)) = mean(UpSampledAmplitude);
            RawValues(i).MaxFFTLogAmpValues(MatchSyllables(k)) = max(UpSampledAmplitude);
            
            BaselineSubtractedValues(i).MeanFFTLogAmpValues(MatchSyllables(k)) = mean(UpSampledAmplitude(SyllStartIndex:SyllEndIndex)) - IndividualBirds(i).AllSyllableLogAmpBaselineAmpValue(MatchSyllables(k));
            BaselineSubtractedValues(i).MaxFFTLogAmpValues(MatchSyllables(k)) = max(UpSampledAmplitude(SyllStartIndex:SyllEndIndex)) - IndividualBirds(i).AllSyllableLogAmpBaselineAmpValue(MatchSyllables(k));
        end
    end
end
