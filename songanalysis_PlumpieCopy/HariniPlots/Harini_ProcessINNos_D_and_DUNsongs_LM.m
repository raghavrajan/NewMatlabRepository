function [DirBout_Stats, UndirBout_Stats, DirUndirBout_Stats] = Harini_ProcessINNos_D_and_DUNsongs_LM(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)
...
for i = 1:length(IndividualBirds),
    ...
    for j = min(IndividualBirds(i).BoutStatistics(:,ConditionColumnIndex)):max(IndividualBirds(i).BoutStatistics(:,ConditionColumnIndex)),
        ...
        % Now, to combine DUN songs with D songs
        if (~isempty(DirUndirBouts{i}{j}))
            DirBouts{i}{j} = [DirBouts{i}{j}(:); DirUndirBouts{i}{j}];
        end
...
print(fullfile(FinalFigureDir, ['D_DUNSongs_Combined_AllBirdsExceptExamples.eps']), '-depsc2', '-r600');
print(fullfile(FinalFigureDir, ['D_DUNSongs_Combined_AllBirdsExceptExamples.png']), '-dpng', '-r600');
...
set(gcf, 'Color', 'w');
set(gcf, 'Units', 'inches');
set(gcf, 'Position', [7 0.5 8 10]);
set(gcf, 'PaperPositionMode', 'auto');
print(fullfile(FinalFigureDir, ['D_DUNSongs_Combined_Fig.3.ResponseExamples_Counts.eps']), '-depsc2', '-r600');
print(fullfile(FinalFigureDir, ['D_DUNSongs_Combined_Fig.3.ResponseExamples_Counts.png']), '-dpng', '-r600');
...
for i = 1:length(Slopes),
    ...
    [H, ResponseSlope_DiffProb(i)] = ttest(Slopes{i}(:,1), Slopes{i}(:,2));
    disp(['Comparing slopes for Dir and undir songs for ', ResponseCategories{i}, ': p = ', num2str(ResponseSlope_DiffProb(i)), ' paired t-test']);
end
...
print(fullfile(FinalFigureDir, ['D_DUNSongs_Combined_ResponseSlopes.eps']), '-depsc2', '-r600');
print(fullfile(FinalFigureDir, ['D_DUNSongs_Combined_ResponseSlopes.png']), '-dpng', '-r600');
