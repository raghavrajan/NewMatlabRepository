function [DirBout_Stats, UndirBout_Stats, DirUndirBout_Stats] = Harini_INMotifNum_MotifDur_DUNAlso(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)
...
for i = 1:length(IndividualBirds),
    ...
    for j = min(IndividualBirds(i).BoutStatistics(:,ConditionColumnIndex)):max(IndividualBirds(i).BoutStatistics(:,ConditionColumnIndex)),
        ...
        % Include DUN bouts with D bouts
...
print(fullfile(FinalFigureDir, ['PercentageofDirSongsWithDistance_DUNAlso.eps']), '-depsc2', '-r600');
print(fullfile(FinalFigureDir, ['PercentageofDirSongsWithDistance_DUNAlso.png']), '-dpng', '-r600');
close all;
...
switch (BirdOption)
    case 'INNum_MotifNumDur'
        Harini_FeatureFits_INNum_MotifNumDur_DUNAlso(DirBout_Stats(BirdsToUse), DirUndirBout_Stats(BirdsToUse), UndirBout_Stats(BirdsToUse), Distances, BirdNames(BirdsToUse));
    case 'Amplitude'
        Harini_FeatureFits_Amplitude_DUNAlso(DirBout_Stats(BirdsToUse), DirUndirBout_Stats(BirdsToUse), UndirBout_Stats(BirdsToUse), Distances, BirdNames(BirdsToUse));
    case 'FF'
        Harini_FeatureFits_FF_DUNAlso(DirBout_Stats(BirdsToUse), DirUndirBout_Stats(BirdsToUse), UndirBout_Stats(BirdsToUse), Distances, BirdNames(BirdsToUse));
end
