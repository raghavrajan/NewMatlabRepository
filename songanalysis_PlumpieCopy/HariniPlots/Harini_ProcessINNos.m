function [DirBout_Stats, UndirBout_Stats, DirUndirBout_Stats] = Harini_ProcessINNos(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)
...
% 
% MinTrialNo = 3;
% StructString = 'INNumber_500ms_Time';
% LabelString = '# of INs';
% ParametricOrNot = 0;
...
% 
% figure; hold on;
% % Now to plot all the model fits and variances
% for i = 1:length(OutputStats),
%     ParametersToPlot = {'GLMFit_Distance' 'GLMFit_Context', 'GLMFit_DistanceContext_Interactions'};
%     for j = 1:length(OutputStats(i).GLMFit_Distance),
%         for k = 1:length(ParametersToPlot),
%             if (eval(['OutputStats(i).', ParametersToPlot{k}, '(j,2)']) < 0.05)
%                 plot(i - 0.2 + k*0.1, eval(['OutputStats(i).', ParametersToPlot{k}, '(j,1)']), 'ro', 'MarkerFaceColor', 'r');
%             else
%                 plot(i - 0.2 + k*0.1, eval(['OutputStats(i).', ParametersToPlot{k}, '(j,1)']), 'ro');
%             end
%         end
%     end
% end
