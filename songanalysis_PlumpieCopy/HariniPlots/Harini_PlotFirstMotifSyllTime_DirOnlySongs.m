function [] = Harini_PlotFirstMotifSyllTime_DirOnlySongs(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)
...
for i = 1:length(IndividualBirds),
    ...
    for j = 1:length(IndividualBirds(i).Conditions),
        ...
        if (length(DirSongIndices) >= 3)
            plot(0:100:max(IndividualBirds(i).BoutStatistics(:, ColumnIndex)), histc(IndividualBirds(i).BoutStatistics(DirSongIndices, ColumnIndex), 0:100:max(IndividualBirds(i).BoutStatistics(:,ColumnIndex)))/length(DirSongIndices), [Colours(mod(j, length(Colours)) + 1), 'o-'], 'LineWidth', 2);
