function [] = Harini_PlotBoutLength(IndividualBirds, BirdNames, BirdOption, FeatureToPlot_ColNo, PlotType, DifferentMicrophones)
...
%   b) Correlation to see if distance and Bout length (msec) is related - spearman's
%   rank correlation 
% =========================================================================
...
Fid = fopen('BoutLength_Correlations.txt', 'w');
...
for i = 1:length(IndividualBirds),
    ...
    for j = UniqueConditions(:)',
        ...
        plot(0:100:max(IndividualBirds(i).BoutStatistics(:, ColumnIndex)), histc(IndividualBird_AllBouts{i}(ConditionIndices, 1), 0:100:max(IndividualBirds(i).BoutStatistics(:,ColumnIndex)))/length(ConditionIndices), [Colours(mod(j, length(Colours)) + 1), 'o-'], 'LineWidth', 2);
    ...
    for j = UniqueConditions(:)',
        ...
        plot(0:100:max(IndividualBirds(i).BoutStatistics(:, ColumnIndex)), histc(IndividualBird_DirectedBouts{i}(ConditionIndices, 1), 0:100:max(IndividualBirds(i).BoutStatistics(:,ColumnIndex)))/length(ConditionIndices), [Colours(mod(j, length(Colours)) + 1), 'o-'], 'LineWidth', 2);
        PlotLegend{i}{end+1} = IndividualBirds(i).Conditions{j};
    end
    ...
    for j = UniqueConditions(:)',
        ...
        plot(0:100:max(IndividualBirds(i).BoutStatistics(:, ColumnIndex)), histc(IndividualBird_UnDirectedBouts{i}(ConditionIndices, 1), 0:100:max(IndividualBirds(i).BoutStatistics(:,ColumnIndex)))/length(ConditionIndices), [Colours(mod(j, length(Colours)) + 1), 'o-'], 'LineWidth', 2);
        PlotLegend{i}{end+1} = IndividualBirds(i).Conditions{j};
    end
...
ylabel('% change in mean Bout length (msec) relative to Undirected song');
