function [UnDirBoutSpikingDetailsIFR, DirBoutSpikingDetailsIFR, UnDirGapSpontActivityIFR, DirGapSpontActivityIFR] = LSINA_CalcIFR(SURecordingDetails)

% This function will calculate the IFR (instantaneous firing rate) for all
% of the bouts and the spontaneous activity periods.
% IFR(t) = 1 / (t(i+1) - t(i)) where t(i) < t <= t(i+1)
% where t(i+1) and t(i) are the times of the i+1'th spike and the i'th
% spike

% The way I will do this is to pull out the spikes belonging to each bout
% and the pull out the nearest spike on either side of the bout and then
% calculate and store IFR based on these. If there is no spike there, I
% will 
