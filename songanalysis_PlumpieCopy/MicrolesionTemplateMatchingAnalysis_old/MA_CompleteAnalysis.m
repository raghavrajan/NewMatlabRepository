...
% 
% for Shuffle = ShufflePercentages,
%     TemplateMatchOutputDir = '/home/raghav/PitchShiftedTemplates/PartShuffledSongComparisons_1_to_100/';
%     [MotifTemplateDir, MotifTemplateFileName, MotifTemplateExt] = fileparts(Parameters.MotifTemplateFileName);
% 
%     TemplateMatchOutputDir = [TemplateMatchOutputDir, MotifTemplateFileName, MotifTemplateExt, '.Shuffle.', num2str(Shuffle), 'percent.TemplateMatchResults', FileSep];
%     if (~exist(TemplateMatchOutputDir, 'dir'))
...
% 
% for Shuffle = ShufflePercentages,
%     TemplateMatchOutputDir = '/home/raghav/PitchShiftedTemplates/PartTimeShuffledSongComparisons_1_to_100/';
%     [MotifTemplateDir, MotifTemplateFileName, MotifTemplateExt] = fileparts(Parameters.MotifTemplateFileName);
% 
%     TemplateMatchOutputDir = [TemplateMatchOutputDir, MotifTemplateFileName, MotifTemplateExt, '.Shuffle.', num2str(Shuffle), 'percent.TemplateMatchResults', FileSep];
%     if (~exist(TemplateMatchOutputDir, 'dir'))
...
ActualTemplateMatchThreshold = max(Parameters.PartMaxShuffledSongMatches{1}); % 50% shuffled songs is used as thresholdls 
...
% ======================= Now to do all the syllable template matching ====
...
%             TempMaxShuffledSongMatches = [TempMaxShuffledSongMatches; cell2mat(cellfun(@max, SyllableParameters.PartShuffledSongMatches{ShuffleIndex}{SyllTemp}{i}, 'UniformOutput', 0))'];
...
%     Results.ActualSyllableTemplateMatchThreshold{SyllTemp} = ActualTemplateMatchThreshold{SyllTemp};
% end
% 
% % % Loading results for other birds songs
% % 
...
%     for SyllTemp = 1:length(Parameters.SyllablePreDirResults{i}),
%         SyllableDirAllMatches{i}{SyllTemp} = cell2mat(Parameters.SyllablePreDirResults{i}{SyllTemp});
%         SyllableDirTotalSongTime{i}{SyllTemp} = cell2mat(Parameters.SyllablePreDirLens{i}{SyllTemp});
%     
...
%         SyllableUnDirTotalSongTime{i}{SyllTemp} = cell2mat(Parameters.SyllablePreUnDirLens{i}{SyllTemp});
% 
...
% 
% for i = 1:length(Parameters.PostDirResults),
%     SyllableDirAllMatches{i + length(Parameters.PreDirResults)}{SyllTemp} = cell2mat(Parameters.SyllablePostDirResults{i}{SyllTemp});
%     SyllableDirTotalSongTime{i + length(Parameters.PreDirResults)}{SyllTemp} = cell2mat(Parameters.SyllablePostDirLens{i}{SyllTemp});
%     
%     if (~isempty(SyllableDirAllMatches{i + length(Parameters.PreDirResults)}))
%         Results.DirAllMatches(Index,:) = [(i + length(Parameters.PreDirResults)) mean(DirAllMatches{i + length(Parameters.PreDirResults)}(:,1)) std(DirAllMatches{i + length(Parameters.PreDirResults)}(:,1))];
%         Results.DirNumMatchesPerSec(Index,:) = [(i + length(Parameters.PreDirResults)) size(DirAllMatches{i + length(Parameters.PreDirResults)},1)*1000/sum(DirTotalSongTime{i + length(Parameters.PreDirResults)})];
% 
% %         subplot(2,1,1);
