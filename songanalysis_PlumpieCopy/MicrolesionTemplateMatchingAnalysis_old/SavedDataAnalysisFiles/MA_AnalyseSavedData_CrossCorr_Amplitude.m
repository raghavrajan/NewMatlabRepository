function [BoutFFT] = MA_AnalyseSavedData_CrossCorr_Amplitude(Parameters, TitleString, varargin)
    ...
    for i = 1:Parameters(ParameterNo).NoPreDays,
        ...
        for j = 1:length(Parameters(ParameterNo).PreDirBoutLens{i}),
            ...
            for k = 1:length(Parameters(ParameterNo).PreDirBoutLens{i}{j}),
                ...
                [c, lags] = xcorr(LogAmplitude, 'coeff');
                
                BoutFFT(ParameterNo).Dir{i}{BoutIndex} = [c; lags];
                
                BoutIndex = BoutIndex + 1;
            end
        end
        LagLengths = cellfun(@length, BoutFFT(ParameterNo).Dir{1});
        MaxLagLengths = max(cellfun(@length, BoutFFT(ParameterNo).Dir{1}));
        DataAvailArray = zeros(length(BoutFFT(ParameterNo).Dir{1}), MaxLagLengths);
        BoutAutoCorr(ParameterNo).Dir{i} = zeros(1, MaxLagLengths);
        
        ArrayMid = (MaxLagLengths + 1)/2;
        for j = 1:length(BoutFFT(ParameterNo).Dir{i}),
            BoutAutoCorr(ParameterNo).Dir{i}((ArrayMid - (LagLengths(j) - 1)/2):(ArrayMid + (LagLengths(j) - 1)/2)) = BoutAutoCorr(ParameterNo).Dir{i}((ArrayMid - (LagLengths(j) - 1)/2):(ArrayMid + (LagLengths(j) - 1)/2)) + BoutFFT(ParameterNo).Dir{1}{j}(1,:);
            DataAvailArray(j, ((ArrayMid - (LagLengths(j) - 1)/2):(ArrayMid + (LagLengths(j) - 1)/2))) = 1; 
        end
        ...
        for j = 1:length(Parameters(ParameterNo).PreUnDirBoutLens{i}),
            ...
            for k = 1:length(Parameters(ParameterNo).PreUnDirBoutLens{i}{j}),
                ...
                BoutFFT(ParameterNo).UnDir{i}{BoutIndex} = [c; lags];
                BoutIndex = BoutIndex + 1;
            end
        end
        LagLengths = cellfun(@length, BoutFFT(ParameterNo).UnDir{1});
        MaxLagLengths = max(cellfun(@length, BoutFFT(ParameterNo).UnDir{1}));
        DataAvailArray = zeros(length(BoutFFT(ParameterNo).UnDir{1}), MaxLagLengths);
        BoutAutoCorr(ParameterNo).UnDir{i} = zeros(1, MaxLagLengths);
        
        ArrayMid = (MaxLagLengths + 1)/2;
        for j = 1:length(BoutFFT(ParameterNo).UnDir{i}),
            BoutAutoCorr(ParameterNo).UnDir{i}((ArrayMid - (LagLengths(j) - 1)/2):(ArrayMid + (LagLengths(j) - 1)/2)) = BoutAutoCorr(ParameterNo).UnDir{i}((ArrayMid - (LagLengths(j) - 1)/2):(ArrayMid + (LagLengths(j) - 1)/2)) + BoutFFT(ParameterNo).UnDir{1}{j}(1,:);
            DataAvailArray(j, ((ArrayMid - (LagLengths(j) - 1)/2):(ArrayMid + (LagLengths(j) - 1)/2))) = 1; 
        end
