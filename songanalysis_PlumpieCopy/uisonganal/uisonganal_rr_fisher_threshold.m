function varargout = uisonganal_rr_fisher_threshold(varargin)

SA_VERSION = 1.4;
% set up sub paths
mypath = fileparts(which('uisonganal_rr')); % This might not be foolproof!
...
%values for display
global d_song time_d_song deriv_song ;  %values used by disp_song
...
SA.param_defaults.save_filt = 0;
SA.param_defaults.save_spect = 0;
...
SA.param_defaults.min_int=7;
SA.param_defaults.min_dur=7;

%values for calculating spectrogram
SA.param_defaults.spect_win_dur=8;
SA.param_defaults.spect_overlap = .9;  %percentage of overlap of specgram window
...
while 1
  ...
  %============ Added by Raghav ===========================================
    ...
    %[smooth] = ASSLCalculateLogAmplitude(rawsong, Fs, Time, FFTWinSize, WinOverlap);
    [smooth] = MA_CalculateLogAmplitudeAronovFee(rawsong, Fs);
    ...
    threshold = ASSLCalculateFisherThreshold(smooth);
    
    song_derivative = diff(smooth);
    song_derivative = (song_derivative - mean(song_derivative))/std(song_derivative);
  ...
  if isempty(onsets)   
    ...
    [onsets, offsets] = ASSLSegmentDataAronovFee(smooth, Fs, min_int, min_dur, threshold);
    %set label values
  ...
  %============ Added by Raghav ===========================================
%   Time = (1:1:length(song_derivative))/Fs * 1000;
%   [pks, locs] = findpeaks(song_derivative, 'MinPeakHeight', 1.5);
%   DerivOnsets = Time(locs);
%    
%   [pks, locs] = findpeaks(-song_derivative, 'MinPeakHeight', 2);
%   DerivOffsets = Time(locs);
%   
%   for i = 1:length(DerivOnsets),
%       if (min(abs(DerivOnsets(i) - onsets)) < 25)
%           continue;
%       end
%       if (i <= length(DerivOffsets))
%           if (min(abs(DerivOffsets(i) - offsets)) < 5)
%             continue;
%           end
%       else
%           continue;
...
%           if ((DerivOnsets(i) >= onsets(j)) && (DerivOnsets(i) <= offsets(j)))
%               Flag = 0;
%               break;
%           end
%           if ((DerivOffsets(i) >= onsets(j)) && (DerivOffsets(i) <= offsets(j)))
%               Flag = 0;
%               break;
%           end
%       end
%       if (Flag == 1)
%           if (~isempty(find(DerivOffsets > DerivOnsets(i), 1, 'first')))
%               Offset = DerivOffsets(find(DerivOffsets > DerivOnsets(i), 1, 'first'));
%               if ((Offset - DerivOnsets(i)) <= 300)
%                   onsets(end+1) = DerivOnsets(i);
...
%       end
%   end
%   [onsets, OnsetIndices] = sort(onsets);
%   offsets = offsets(OnsetIndices);
%   labels = labels(OnsetIndices);
  ...
  if batch_disp == 1 | batch_print == 1
    ...
    deriv_song = song_derivative(1:step:length(song_derivative));
    if do_zc
    ...
    if (min(d_song) < 0)
        g_ymin=min(d_song)*1.2;
    else
        g_ymin=min(d_song)*0.8;
    end
    if (max(d_song) < 0)
        g_ymax=max(d_song)*0.8;
    else
    ...
    if (g_ymin > g_ymax)
        set(h_main_amp,'ylim',[g_ymax g_ymin]);
    else
