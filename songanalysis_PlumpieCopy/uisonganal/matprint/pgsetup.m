...
if(nargin<1)
	...
	uicontrol(PSFigure,'Style','Edit',...
						...
						'Tag','Bottom',...
						'Callback','pgsetup(''Margins'',''Bottom'')');
	uicontrol(PSFigure,'Style','Text',...
						'Units','Points',...
						'BackgroundColor',Background,...
						'ForegroundColor',Foreground,...
	...
	uicontrol(PSFigure,'Style','Edit',...
						...
						'Tag','Right',...
						'Callback','pgsetup(''Margins'',''Right'')');
	...
	set(PatchObj,'ButtonDownFcn','pgsetup(''Resize'')');
	...
	if(nargin<2)
		...
		if(cp(1,1)<ext(1)+.2*ext(3))
			if(cp(1,2)<ext(2)+.2*ext(4)) 		% Lower Left Corner
				set(gcf,'WindowButtonMotionFcn','pgsetup(''Resize'',''Corner'',1)');
			elseif(cp(1,2)>ext(2)+.8*ext(4)) 	% Upper Left Corner
				set(gcf,'WindowButtonMotionFcn','pgsetup(''Resize'',''Corner'',2)');
			else								% Left Side
				set(gcf,'WindowButtonMotionFcn','pgsetup(''Resize'',''Side'',1)');
			end
		elseif(cp(1,1)>ext(1)+.8*ext(3))
			if(cp(1,2)<ext(2)+.2*ext(4)) 		% Lower Right Corner
				set(gcf,'WindowButtonMotionFcn','pgsetup(''Resize'',''Corner'',4)');
			elseif(cp(1,2)>ext(2)+.8*ext(4)) 	% Upper Right Corner
				set(gcf,'WindowButtonMotionFcn','pgsetup(''Resize'',''Corner'',3)');
			else								% Right Side
				set(gcf,'WindowButtonMotionFcn','pgsetup(''Resize'',''Side'',3)');
			end
		elseif(cp(1,2)>ext(2)+.8*ext(4))		% Top Side
				set(gcf,'WindowButtonMotionFcn','pgsetup(''Resize'',''Side'',2)')
		elseif(cp(1,2)<ext(2)+.2*ext(4))		% Bottom Side
				set(gcf,'WindowButtonMotionFcn','pgsetup(''Resize'',''Side'',4)')
		else									% Center
			set(gca,'UserData',cp);
			set(gcf,'WindowButtonMotionFcn','pgsetup(''Resize'',''Move'')');
		end
		set(PatchObj,'erasemode','xor');
		set(gcf,'WindowButtonUpFcn','pgsetup(''Resize'',''Up'')');
