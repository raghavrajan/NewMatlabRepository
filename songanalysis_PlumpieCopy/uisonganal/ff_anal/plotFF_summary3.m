function plotFF_summary3(pre2, post2wk, bird, note);
...
%norm_pre1 = [pre1(1:length(pre1)-8),'regression.mat'];
norm_pre2 = [pre2(1:length(pre2)-8),'regression.mat'];
%norm_post1d = [post1d(1:length(post1d)-8),'regression.mat'];
norm_post2wk = [post2wk(1:length(post2wk)-8),'regression.mat'];
%norm_post4wk = [post4wk(1:length(post4wk)-8),'regression.mat'];
%norm_post8wk = [post8wk(1:length(post8wk)-8),'regression.mat'];
...
X = [1:2]; 
dir_mean = [dir_pre2_mean, dir_post2wk_mean];
undir_mean = [undir_pre2_mean, undir_post2wk_mean];
dir_sem = [dir_pre2_sem, dir_post2wk_sem];
undir_sem = [undir_pre2_sem, undir_post2wk_sem];
...
axis([0.5 2.5 0.995*min([dir_mean,undir_mean]) 1.005*max([dir_mean,undir_mean])]);
xt =[]; set(gca, 'XTick',xt); 
text(0.9,0.993*min([dir_mean,undir_mean]), 'pre1d')
text(1.9,0.993*min([dir_mean,undir_mean]), 'post2wk')

% plot CV of FF
dir_cv = [dir_pre2_cv, dir_post2wk_cv];
undir_cv = [undir_pre2_cv, undir_post2wk_cv];
subplot(2,2,3); plot(X, dir_cv, '-or'); hold on
plot(X, undir_cv, '-ob')
title(['CV of fundamental frequency'])
axis([0.5 2.5 0 1.05*max([dir_cv,undir_cv])]);
xt =[]; set(gca, 'XTick',xt); 
text(0.9,-.1, 'pre1d')
text(1.9,-.1, 'post2wk')
...
% plot SD of FF
dir_std = [dir_pre2_std, dir_post2wk_std];
undir_std = [undir_pre2_std, undir_post2wk_std];
subplot(2,2,2); 
plot(X, dir_std, '-or'); hold on
plot(X, undir_std, '-ob')
title(['SD of fundamental frequency'])
axis([0.5 2.5 0 1.05*max([dir_std,undir_std])]);
xt =[]; set(gca, 'XTick',xt); 
text(0.9,-.5, 'pre1d')
text(1.9,-.5, 'post2wk')

% plot SD of normalized FF
dir_norm_std = [dir_pre2_norm_std, dir_post2wk_norm_std];
undir_norm_std = [undir_pre2_norm_std, undir_post2wk_norm_std];
subplot(2,2,4); 
plot(X, dir_norm_std, '-or'); hold on
plot(X, undir_norm_std, '-ob')
title(['SD of normalized fundamental frequency'])
axis([0.5 2.5 0 1.05*max([dir_norm_std,undir_norm_std])]);
xt =[]; set(gca, 'XTick',xt); 
text(0.9,-.5, 'pre1d')
text(1.9,-.5, 'post2wk')
