function autocorr_ff_pinterp_plotnotes_RR(RawDataDir, batch_in,note, ff_low, ff_hi,FileType, PlotOption) % feature added by Raghav as an argument
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            acwin=autocorr(ZeroLagIndex+round(Fs/ff_hi):ZeroLagIndex+round(Fs/ff_low)); 
            [max1,loc]=max(acwin);
            if loc==1 | loc==length(acwin)
                peak=loc;   %parabolic interpolation req at least 3 points
            else
                [peak, max1]=pinterp([loc-1;loc;loc+1],[acwin(loc-1);acwin(loc);acwin(loc+1)]);
            end     %peak values are real number
            period=peak+(round(Fs/ff_hi) - 1);   
            
            ff=Fs/period
            SpectrogramPlotFig = gcf;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
           if (strfind(PlotOption, 'on'))
               figure;
               plot(lags,autocorr,'b');
               hold on;
               plot(lags(ZeroLagIndex+round(Fs/ff_hi):ZeroLagIndex+round(Fs/ff_low)),acwin,'r');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%            
               plot(period,max1,'ks', 'MarkerSize', 8, 'MarkerFaceColor', 'k');
               axis tight;
               Temp = axis;
               Temp = [-10 lags(ZeroLagIndex + round(Fs/ff_low))+10 0.99*Temp(3) 1.01*Temp(4)];
               axis(Temp);
               set(gcf, 'Position', [1017 664 560 300]);
        
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%            
        
                figure(SpectrogramPlotFig);
                set(gcf, 'Position', [281 662 560 300]);
