function varargout = uisonganal_rr_no_deriv(varargin)
...
while 1
  ...
  %============ Added by Raghav ===========================================
    ...
    [S, F, T, P] = spectrogram(filtsong, hamming(WinSize), WinOverlap, WinSize, Fs);
    ...
    smooth = (smooth - mean(smooth))/std(smooth);
    smooth = spline(T, smooth, linspace(T(1), T(end), length(filtsong)));
  ...
  if isempty(onsets)   
    ...
    if save_notes == 1; 
      ...
      [DerivOnsets, DerivOffsets] = segment(smooth, Fs, min_int, min_dur, threshold);
  ...
  if batch_disp == 1 | batch_print == 1
    ...
    [h_amp_plot, h_labels] = disp_song_no_deriv;
