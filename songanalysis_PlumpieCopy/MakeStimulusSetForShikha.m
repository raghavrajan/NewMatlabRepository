function [] = MakeStimuliForShikha(StimulusFileList, NumRepetitions, MinInterval, MaxInterval, OutputVolume, OutputFileName)
...
for i = 1:length(Temp),
    ...
    if (i <= NumStimuli)
        ...
        NoiseStimulus{i} = (rand(size(Stimulus{i})) * (SortedStim(round(0.98*length(SortedStim))) - SortedStim(round(0.02*length(SortedStim))))) + SortedStim(round(0.02*length(SortedStim)));
        NoiseStimulus{i} = NoiseStimulus{i}(:);
        NoiseStimulus{i} = NoiseStimulus{i}/max(abs(NoiseStimulus{i}));
        NoiseStimDur(i) = length(NoiseStimulus{i})/Fs;
    else
        StimOnsets{i} = Info{2};
        StimOffsets{i} = Info{3};
...
StimOrder = repmat([1:1:2*NumStimuli]', NumRepetitions, 1);
...
for i = 1:length(StimOrder),
    ...
    if (StimOrder(i) > length(Stimulus))
        OutputStimulus = [OutputStimulus; TempIntervalData; OutputVolume*NoiseStimulus{StimOrder(i) - length(Stimulus)}];
        fprintf(Fid, 'Time %6.4fs: Noise Stimulus - #%i - %s\n', ElapsedTime + Intervals(i), StimOrder(i) - length(Stimulus), StimFileName{StimOrder(i) - length(Stimulus)});
        ElapsedTime = ElapsedTime + Intervals(i) + NoiseStimDur(StimOrder(i) - length(Stimulus));
    else
        OutputStimulus = [OutputStimulus; TempIntervalData; OutputVolume*Stimulus{StimOrder(i)}];
    ...
    if (i == length(StimOrder))
        ...
        OutputStimulus = [OutputStimulus; Silence];
        if (~isempty(OutputStimulus))
            wavwrite(OutputStimulus(1:(length(OutputStimulus) - round(Fs*1))), Fs, 16, OutputStimFileName{OutputStimulusFileIndex});
        end
        OutputStimulus(1:(length(OutputStimulus) - round(Fs*1))) = [];
