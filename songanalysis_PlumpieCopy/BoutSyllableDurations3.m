function [Bout] = BoutSyllableDurations3(DirectoryName, FileList, FileType, PlotOption, ChooseBouts, min_int, min_dur, GetRidNoise)
...
while (ischar(SongFile(1)))
    ...
    try
        ...
        EntropyTime = linspace(SongTime(1), SongTime(end), length(m_Entropy));
        ...
        for WInds = 2:64,
            WindowIndices(WInds,:) = WindowIndices(WInds-1,:) + 1;
        end
        VarPower = var(Song(WindowIndices));
        VarPower = spline(SongTime(WindowIndices(1,:)), VarPower, SongTime);
        
        clear WindowIndices;
        
        m_PGFull = spline(EntropyTime, m_PitchGoodness, SongTime);
        m_PGFull = reshape(m_PGFull, length(m_PGFull), 1);
        
        m_EntropyFull = spline(EntropyTime, m_Entropy, SongTime);
        Power = reshape(Power, length(Power), 1);
        m_EntropyFull = reshape(m_EntropyFull, length(m_EntropyFull), 1);
        VarPower = reshape(VarPower, length(VarPower), 1);
        Obj = gmdistribution.fit([Power VarPower], 2, 'Start', 'randSample', 'Replicates', 10);
...
%        Threshold = (NoiseMSD(1) + SoundMSD(1))/2;
        [MaxVal, MaxIndex] = max(Obj.mu(:,2));
        Threshold = MaxIndex;
        Idx = cluster(Obj, [Power VarPower]);
