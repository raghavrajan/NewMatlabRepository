function [Bout] = BoutMotifSpectralMatch(DirectoryName, FileList, FileType, PlotOption, MotifTemplate, StretchValues, GetRidNoise, BoutLimits, varargin)

if (nargin > 8)
    MotifTemplateRawData = varargin{1};
    MotifTemplateFs = varargin{2};
end
InterBoutInterval = 1;
MinBoutLen = 1;
...
%                 end
            ...
            Bout.FileName{BoutIndex} = SongFile;
            Bout.FileLength{BoutIndex} = SongTime(end);
            Bout.BoutSeqMatch = [];
    ...
    if (strfind(PlotOption, 'on'))
        subplot(2,1,1);
        PlotSpectrogram2(DirectoryName, SongFile, FileType);
        hold on;
%        plot(SongTime, (Power + (abs(min(Power))))*100);
        ...
        if (~isempty(Bouts))
            for j = 1:size(Bouts,1),
                ...
                plot(Bout.T{BoutIndex - j}(1:length(Bout.MaxBoutSeqMatch{BoutIndex - j})), (Bout.MaxBoutSeqMatch{BoutIndex-j}/max(Bout.MaxBoutSeqMatch{BoutIndex-j})) *7000, 'k', 'LineWidth', 2);
            end
        end
        subplot(2,1,2);
        hold on;
        for j = 1:size(Bouts,1),
            plot(Bout.T{BoutIndex - j}(1:length(Bout.MaxBoutSeqMatch{BoutIndex - j})), Bout.MaxBoutSeqMatch{BoutIndex-j}, 'k', 'LineWidth', 2);
        end
        axis tight;
